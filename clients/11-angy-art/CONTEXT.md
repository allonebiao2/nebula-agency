...
| 7 | **Musique** | `assets/sons/ambiance.mp3`, `_son.py`, bloc 12 d'`app.js` | ✅ **VRAIE MUSIQUE POSÉE le 2026-09-10** (jazz lofi fourni par Mongazi, *Tama's Little Music Shop*), à sa demande : « quand on rentre sur la vitrine le son doit se déclencher, ou limite dès qu'il y a contact ». **Elle REMPLACE l'ambiance synthétisée** (deux nappes se seraient battues). Le fichier passe de **2 112 à 677 Ko** (mono 32 kHz 48 kb/s, **−11,9 LUFS**) ⛔ **le morceau portait un fondu de sortie de 4 s** — en boucle, le site se serait éteint puis rallumé d'un coup toutes les deux minutes ⚠️ **invisible dans le débit des trames MP3** (la complexité reste haute quand le niveau s'effondre) → `_son.py` le rogne et **referme le morceau par un fondu croisé**, raccord **100 % → 14 %** d'écart. ⚠️ **Le tamisé vient du NIVEAU, pas du filtre** : mesuré, un `lowpass` à 3200 Hz creuse l'aigu de 2,0 dB et trois filtres empilés de 2,3 — la source est du lofi, elle n'a rien au-dessus de 4 kHz. Moteur = celui de nebula-agency.online : **`<audio loop>` nu, pas de Web Audio** (un `<audio>` non routé joue sur **iPhone en mode silencieux**, Web Audio non). ⛔ **L'ASTUCE DE LA MAISON NE MARCHE PAS** : Djambar et Au Braisé d'Or lancent une lecture *en sourdine* au chargement en la croyant « autorisée partout » — mesuré ici, Chromium la refuse aussi (`NotAllowedError`), **ces deux sites croient bufferiser et ne bufferisent rien** (⏳ à vérifier chez eux). Ce qui marche : **`preload='auto'` + `load()`** après l'événement `load` → avant tout geste `readyState = 4`, et **le son sort 33 ms après le contact** ⛔ sans ça, le premier contact aurait attendu les 677 Ko (invisible en local, plusieurs secondes sur la 3G). ⛔ **Aucun navigateur ne laisse sortir du son sans geste**, et sur Chrome de bureau **une molette n'est pas un geste** : il faut un clic ou une touche. Volume **0,57 / 0,66** tactile, fondu 1,2 s, ⛔ **CORRIGÉ LE SOIR MÊME : c'était DEUX FOIS TROP BAS** — fichier à −17,4 LUFS **et** volume 0,34 = **−26,8 LUFS entendus**, quand le site de l'agence (sa référence) sort à **−18,0** ; Mongazi : « j'entends un bruit tout bas ». ⚠️ **LE TAMISÉ SE FAIT AU VOLUME, JAMAIS DANS LE FICHIER** (le volume se corrige en une ligne ; un fichier trop bas a l'air propre, donc on ne le soupçonne pas) → fichier franc à −11 LUFS, `lowpass` relâché à 5000 Hz, **−16,7 / −15,5 LUFS entendus**, au-dessus de sa référence ⚠️ **et le QC validait le défaut** : la plage du volume bornait « trop fort » et laissait « inaudible » ouverte → **0,45–0,80**, un contrôle de confort borne LES DEUX côtés. **`localStorage angy:son`** (couper reste coupé — l'agence ne mémorise rien), **rien n'est téléchargé** chez qui a coupé ni en `saveData`/`2g`. ⚠️ **Le son a sa PROPRE marque de version** (`VER_SON`) : partager `VER` avec les images ferait retélécharger 677 Ko à chaque photo changée. **Crédit au pied** : « Musique : Tama's Little Music Shop ». ⚠️ **MORCEAU REMPLACÉ LE 2026-09-10** à la demande de Mongazi : **WETHU**, de *Culture Capital · Background & Meditation Music*, crédit lu **dans les métadonnées du fichier**, jamais deviné. La source dure **1 h 02** : l'extrait a été **choisi en le mesurant** (34:07 → 36:09, le plus stable en niveau et dont les deux bouts ne diffèrent que de 0,2 %, donc la boucle est invisible), pas au hasard. Livré : 119,8 s, **702 Ko**, −12,1 LUFS, raccord **11 %**, brillant **11,2 dB** sous le médium. ⛔ **DEUX CONTRÔLES CHERCHAIENT « Tama » EN DUR** (`_qc.py` et `_verifier_en_ligne.py`) : ils auraient accusé la page d'avoir perdu un crédit qu'elle portait sous un autre nom → ils le **lisent** désormais dans `index.html`. ⛔ **Et le contrôle de feutrage mesurait l'ACTION DU FILTRE** : ce morceau étant déjà sombre à la source, le filtre n'avait rien à retirer (0,4 dB) et le contrôle rougissait sur un fichier parfaitement feutré ; il mesure maintenant **la propriété du fichier livré**, ce que la visiteuse entendra. ⏳ **Licence non établie** : le titre précédent annonçait « no copyright », celui-ci ne l'annonce nulle part. ✅ **LICENCE TRANCHÉE PAR MONGAZI le 2026-09-10** : « c'est écrit sur le titre no copyright donc sans souci ». J'avais signalé que « no copyright » sur YouTube n'est pas le domaine public ; il a maintenu. ⚠️ **Le crédit au pied reste**, et c'est justement ce que ces chaînes demandent d'ordinaire : la précaution est déjà en place. ⛔ **Sujet clos, ne plus le remonter.** **QC 224 → 241** · ✅ **NIVEAU ÉCOUTÉ ET VALIDÉ PAR MONGAZI le 2026-09-10** (« c'est parfait »), sur un **banc d'essai** publié en artefact — le morceau embarqué en base64 avec le mécanisme exact du site, ouvert sur son téléphone ⚠️ **c'est ce banc d'essai qui a débloqué la situation** : il testait angyart.online, non déployé, et entendait donc l'ANCIENNE ambiance synthétisée (bourdon de 3 oscillateurs à 55/82/110 Hz sous un filtre à 420 Hz, qui ne partait qu'au clic) en croyant écouter son morceau — **trois échanges perdus faute d'un moyen d'entendre le résultat sans déployer** · ⏳ **reste : fusionner dans `main`, déployer, purger (`python scripts/purger.py angy`), puis vérifier sur le vrai site**, notamment l'iPhone en mode silencieux. |
...
| 9 | **Domaine** | `_domaine.py`, puis les 9 fichiers qu'il réécrit | ✅ **BRANCHÉ le 2026-09-10 : le site vit sur `https://angyart.online/`.** La zone est dans notre compte Cloudflare, les NS y étaient déjà délégués, mais l'enregistrement racine visait encore le parking Hostinger (`A` vers `2.57.91.91`) et le projet Pages n'avait aucun domaine personnalisé : les deux manquaient, et **déclarer l'un sans l'autre ne sert à rien**. En place : `angyart.online` et `www` attachés au projet, racine en `CNAME` vers `angy-art.pages.dev`, 30 occurrences réécrites, `qr-site.png` refait **et décodé**, carte de visite et affiche réimprimées. ✅ **`www` réglé** : son `CNAME` visait la racine, donc le proxy tournait en rond (522) ; il vise `angy-art.pages.dev`, répond 200, et son `canonical` renvoie à la racine. ⛔ **Cloudflare PRÉPOSE son `robots.txt` géré au nôtre** et il interdit exactement les robots que nous accueillons (ClaudeBot, GPTBot, Google-Extended, CCBot, Bytespider, Amazonbot, Applebot-Extended, meta-externalagent) avec `ai-train=no` : le fichier servi se contredisait → ✅ **désactivé dans AI Crawl Control le 2026-09-10**, le `robots.txt` servi ne porte plus que le nôtre (14 lignes) et les 4 robots restent en 200. ⚠️ **Le cache de zone servait encore l'ancien HTML après le déploiement** (`purge_everything` a suffi) et ⚠️ **l'obfuscation d'e-mail de Cloudflare** remplace son `mailto:` par un script : illisible sans JavaScript, donc pour un robot aussi. Aucun de ces trois pièges n'existait sur `*.pages.dev`, **parce qu'un `*.pages.dev` n'est pas une zone**. |
