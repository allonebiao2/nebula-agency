...
def transcribe_bytes(audio: bytes, content_type: str | None = None) -> str | None:
    """Transcrit des octets audio via Groq Whisper (compatible OpenAI). None si KO."""
    if not (settings.groq_api_key and audio):
        return None
    ext = "ogg"
...
def transcribe_audio(media_url: str, content_type: str | None = None) -> str | None:
    ...
    if not (settings.groq_api_key and media_url):
        return None
    # Télécharger l'audio depuis Twilio (auth requise, suit la redirection)
    try:
        auth = None
        ...
        return None
    return transcribe_bytes(audio, content_type)
