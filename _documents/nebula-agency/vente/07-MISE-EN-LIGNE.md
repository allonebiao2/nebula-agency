...
| 5 | **Rappel de renouvellement** : ~~bloqué~~ **le modèle de données est implémenté et testé** (table `subscriptions`, commission 20 % à l'encaissement, 6 endpoints). Reste le workflow n8n et la variable `NAFF_CRON_KEY` sur Railway | `server.py` + `10-RELANCE-RENOUVELLEMENT.md` |
