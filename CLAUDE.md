...
| 10 | Hillary | **HILLARY M. STYL** — maison de couture (monogramme H.M.S) · prêt-à-porter par tailles **+ sur-mesure** · magenta `#E6007E` + noir, Archivo + Manrope | **Vitrine + moteur de commande livrée 2026-07-31** (`clients/10-hillary-m-styl/vitrine.html`, 120 Ko, logo base64 WebP déclaré 1 seule fois) : catalogue à 2 onglets, fiche produit modale, **tunnel en 4 étapes** (taille OU 8 mesures femme/homme → retrait atelier ou expédition avec **frais par pays** → délai normal/express → coordonnées) + récapitulatif chiffré en direct → WhatsApp structuré · accueil personnalisé (moment de la journée + prénom mémorisé) · QC : 0 débordement 390/768/1440, 0 erreur JS, cibles ≥44px, 0 image externe · **MISE EN LIGNE 2026-08-01 : https://hillary-m-styl.pages.dev** (Cloudflare Pages `hillary-m-styl`) — **vrai n° WhatsApp posé** (fourni par Mongazi), plus aucun placeholder public (atelier = « point de retrait donné sur WhatsApp », carte « Horaires » devenue « Confection »), canonique + og:url posés, **affiche A4 + 2 QR** (site + WhatsApp pré-rempli, décodés pour vérification) dans `assets/docs/` · scripts `_outils/_apply_infos.py` + `_build_affiche.py` (UTF-8, idempotents) · reste : **frais d'expédition réels**, délais de confection, vraies pièces + prix, **photos** (aucune fournie), adresse de l'atelier | **+229 51 37 47 93** |
