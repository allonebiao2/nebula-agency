-- ════════════════════════════════════════════════════════════════════════════
-- MINUIT · LES LETTRES ET LEUR CAISSE
--
...
-- `naff`, et `public.boussole_proto_etat`). MINUIT est le quatrième. Il prend
...
CREATE SCHEMA IF NOT EXISTS minuit;
...
CREATE TABLE IF NOT EXISTS minuit.lettres (
...
CREATE INDEX IF NOT EXISTS lettres_etat ON minuit.lettres (etat, cree_le DESC);
...
-- navigateur.
CREATE TABLE IF NOT EXISTS minuit.paiement_session (
...
CREATE UNIQUE INDEX IF NOT EXISTS minuit_evenement_unique
  ON minuit.paiement_evenement (fournisseur, evenement_id)
...
CREATE OR REPLACE FUNCTION public.minuit_deposer(
  p_jeton text, p_palier text, p_prix numeric, p_donnees jsonb,
  p_whatsapp text, p_etat text, p_expire timestamptz)
RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path = minuit, public AS $$
BEGIN
  INSERT INTO minuit.lettres (jeton, palier, prix, donnees, whatsapp, etat, expire_le, paye_le)
...
-- lettre.
CREATE OR REPLACE FUNCTION public.minuit_lire(p_jeton text)
RETURNS TABLE (donnees jsonb, etat text, expire_le timestamptz, retire_le timestamptz)
LANGUAGE sql SECURITY DEFINER SET search_path = minuit, public AS $$
  SELECT l.donnees, l.etat::text, l.expire_le, l.retire_le
    FROM minuit.lettres l
...
CREATE OR REPLACE FUNCTION public.minuit_paiement_attendu(p_jeton text)
...
CREATE OR REPLACE FUNCTION public.minuit_paiement_journal(
...
BEGIN
  INSERT INTO minuit.paiement_evenement
...
-- du vendredi.
CREATE OR REPLACE FUNCTION public.minuit_ouvrir(p_jeton text, p_jours integer)
RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path = minuit, public AS $$
DECLARE v_fait integer;
BEGIN
  UPDATE minuit.lettres
     SET etat = 'vivante',
...
CREATE OR REPLACE FUNCTION public.minuit_retirer(p_jeton text)
RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path = minuit, public AS $$
DECLARE v_fait integer;
BEGIN
...
CREATE OR REPLACE FUNCTION public.minuit_menage()
RETURNS integer
LANGUAGE plpgsql SECURITY DEFINER SET search_path = minuit, public AS $$
DECLARE v_fait integer;
BEGIN
...
BEGIN
  FOR f IN
    SELECT format('%I.%I(%s)', n.nspname, p.proname, pg_get_function_identity_arguments(p.oid))
     ...
     WHERE n.nspname = 'public' AND p.proname LIKE 'minuit\_%'
  LOOP
...
ALTER TABLE minuit.lettres            ENABLE ROW LEVEL SECURITY;
ALTER TABLE minuit.paiement_session   ENABLE ROW LEVEL SECURITY;
ALTER TABLE minuit.paiement_evenement ENABLE ROW LEVEL SECURITY;
