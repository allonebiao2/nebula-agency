...
export function chartEvolution(serie) {
  ...
  const dots = (vals, col) => vals.map((v, i) => serie[i].unites > 0
    ? `<circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="3.4" fill="${col}" filter="url(#glow${id})"/><circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="1.6" fill="#fff"/>` : '').join('');
  ...
  return `<svg viewBox="0 0 ${W} ${H}" class="chart-svg" role="img" aria-label="Évolution du revenu et du bénéfice">
    ...
    <path d="${benPath}" fill="none" stroke="${COL.pos}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow${id})"/>
    ${dots(revs, COL.rev)}${dots(bens, COL.pos)}
    ${endLbl(revs[last], COL.rev, -6)}${endLbl(bens[last], COL.pos, 12)}
    ${labels}${legend}
  </svg>`;
}
...
export function chartHero(D) {
  ...
  return `<svg viewBox="0 0 ${W} ${H}" class="chart-svg" role="img" aria-label="Évolution du chiffre d'affaires et du bénéfice">
    ...
    <path d="${benPath}" fill="none" stroke="${COL.pos}" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" filter="url(#glow${id})"/>
    ${dot(revs, COL.rev)}${dot(bens, COL.pos)}
    ${endLbl(revs[n - 1], COL.rev, -6)}${endLbl(bens[n - 1], COL.pos, 12)}
    ${labels}
  </svg>`;
}
...
export function sparklineRaw(values, opts = {}) {
  ...
  return `<svg viewBox="0 0 ${W} ${H}" class="chart-svg spark" preserveAspectRatio="none" aria-hidden="true">
    <defs><linearGradient id="sp${id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${col}" stop-opacity=".26"/><stop offset="1" stop-color="${col}" stop-opacity="0"/></linearGradient></defs>
