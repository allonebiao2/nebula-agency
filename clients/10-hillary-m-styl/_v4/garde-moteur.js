...
var PIECES = [
  ...
  {id:"h19", cat:"sm", nom:"Robe d'été", type:"robe_ovale", tag:"Plage",
   img:"piece-ete.webp",
   prix:35000, jmin:14, jmax:14, expPrix:45000, expMin:2, expMax:4,
   eur:52, usd:63, eurExp:67, usdExp:81,
...
/* ⚠️ 2026-09-04 — CE QU'HILLARY DOIT SAVOIR AVANT DE LANCER UNE COUPE.
   Sa liste, mot pour mot : lieu de résidence, nom, prénom, numéro de
   téléphone, lieu d'expédition ou de livraison — ou le retrait à l'atelier.
   Trois d'entre elles manquaient ou étaient facultatives : le nom pouvait
   rester vide, le numéro pouvait être remplacé par un email, et le lieu de
   résidence n'existait pas (seule la ville de LIVRAISON était demandée, et
   seulement en expédition). Une commande arrivait donc à l'atelier sans
   moyen d'appeler la cliente, alors qu'Hillary annonce l'appeler. */
var cmd = {
  actif:false, etape:1, mode:null, pays:null, ville:"", repere:"",
  prenom:"", nom:"", tel:"", mail:"", residence:"", note:""
};
...
function ouvrirCommande(){
  ...
  cmd.residence = cmd.residence || m.residence || "";
  etat = null;
  fermerPanier();
  var t = totaux();
...
/* la marque discrète d'un champ obligatoire : une étoile, et son sens dit
   à voix haute pour qui n'a que le lecteur d'écran */
var REQ = ' <span class="req" aria-hidden="true">*</span><span class="vhs"> (obligatoire)</span>';
...
function vueLivraison(){
  ...
  if(cmd.mode === "expedition"){
    ...
    /* ⚠️ LA VILLE EST OBLIGATOIRE DEPUIS LE 2026-09-04. Avant, choisir un
       pays suffisait pour avancer : une commande pouvait partir avec
       « Côte d'Ivoire » pour toute adresse. Hillary demande le lieu de
       livraison, pas la zone tarifaire. */
    h += '<div class="fd" style="margin-top:12px"><label for="f_ville">Ville de livraison'+REQ+'</label>'+
         '<input id="f_ville" type="text" placeholder="Cotonou, Abidjan, Dakar…" value="'+esc(cmd.ville)+'"></div>';
    /* ⚠️ AU BÉNIN, UNE ADRESSE EST UN REPÈRE, PAS UNE RUE. Demander « votre
       adresse » ne donne rien d'exploitable ; demander le quartier et ce
       qu'on voit depuis la porte donne une livraison qui arrive. Facultatif :
       le numéro est obligatoire, l'atelier peut toujours préciser au
       téléphone, et un champ de plus avant le bouton coûte des commandes. */
    h += '<div class="fd" style="margin-top:12px"><label for="f_repere">Quartier ou point de repère '+
         '<span style="font-weight:500;text-transform:none">(facultatif)</span></label>'+
         '<input id="f_repere" type="text" placeholder="Fidjrossè, en face de la pharmacie" value="'+esc(cmd.repere)+'"></div>';
  }
  if(cmd.mode === "retrait"){
  ...
  return h + ligneManque();
}

/* ⚠️ CE QUI MANQUE ENCORE, EN TOUTES LETTRES.
   Un bouton grisé sans explication laisse la cliente chercher lequel des
   champs le retient. On nomme ce qui manque, dans l'ordre où les champs se
   lisent, aux DEUX étapes qui en ont : la livraison et les coordonnées.
   ⛔ La première version ne le faisait qu'à l'étape 2, et l'étape 1 gardait
      son bouton mort et muet. Ça s'est vu sur une capture, pas dans le code. */
function manqueLivraison(){
  var m = [];
  if(cmd.mode === "expedition"){
    if(!cmd.pays)          m.push("votre pays");
    if(!cmd.ville.trim())  m.push("la ville de livraison");
  }
  return m;
}

/* ce qui manque à l'étape affichée, quelle qu'elle soit */
function manqueEtape(){
  if(!cmd.actif) return [];
  if(cmd.etape === 1) return cmd.mode ? manqueLivraison() : [];
  if(cmd.etape === 2) return manqueCoord();
  return [];
}

/* ⚠️ « COMMENCE » N'EST PAS « A CHOISI UN MODE ». Le premier jet allumait la
   ligne dès le clic sur « Expédition », c'est-à-dire au-dessus de deux champs
   encore vides que la cliente s'apprête à remplir : on lui reprochait de ne
   pas avoir fait ce qu'elle était en train de faire. Elle a commencé quand
   elle a rempli QUELQUE CHOSE. */
function commenceEtape(){
  if(cmd.etape === 1) return !!cmd.pays || !!cmd.ville.trim();
  if(cmd.etape === 2) return manqueCoord().length < 4;
  return false;
}

/* la ligne se tait tant que la cliente n'a rien commencé : accueillir
   quelqu'un par la liste de ce qu'il n'a pas fait est une façon de le faire
   partir. « Commencé » = avoir choisi un mode, ou rempli un champ. */
function ligneManque(){
  var dit = commenceEtape() && manqueEtape().length ? texteManque() : "";
  return '<div class="err doux'+(dit ? " on" : "")+'" id="err">'+esc(dit)+'</div>';
}
function texteManque(){
  var m = manqueEtape();
  return m.length ? "Encore : " + m.join(" · ") : "";
}

function manqueCoord(){
  var m = [];
  if(!cmd.prenom.trim())    m.push("votre prénom");
  if(!cmd.nom.trim())       m.push("votre nom");
  if(!cmd.tel.trim())       m.push("votre numéro");
  if(!cmd.residence.trim()) m.push("votre lieu de résidence");
  return m;
}

function vueCoord(){
  /* ⚠️ LE LIEU DE RÉSIDENCE N'EST PAS LE LIEU DE LIVRAISON, et Hillary
     demande les deux. Le plus souvent ils se confondent, mais pas toujours :
     une cliente de Cotonou peut faire livrer sa sœur à Abidjan. On ne le
     suppose donc pas, on propose de le recopier en un geste. */
  var repique = (cmd.mode === "expedition" && cmd.ville.trim()
                 && cmd.residence.trim() !== cmd.ville.trim())
    ? '<button class="chip" type="button" id="btMemeLieu">J\'habite à '+esc(cmd.ville.trim())+'</button>'
    : '';
  ...
  return '<div class="stitle">Vos coordonnées</div>'+
    ...
    '<div class="mes">'+
      '<div class="fd"><label for="f_prenom">Prénom'+REQ+'</label><input id="f_prenom" type="text" autocomplete="given-name" value="'+esc(cmd.prenom)+'" placeholder="Votre prénom"></div>'+
      '<div class="fd"><label for="f_nom">Nom'+REQ+'</label><input id="f_nom" type="text" autocomplete="family-name" value="'+esc(cmd.nom)+'" placeholder="Votre nom"></div>'+
      '<div class="fd"><label for="f_tel">Numéro de téléphone'+REQ+'</label><input id="f_tel" type="tel" inputmode="tel" autocomplete="tel" value="'+esc(cmd.tel)+'" placeholder="+229 01 …"></div>'+
      '<div class="fd"><label for="f_residence">Lieu de résidence'+REQ+'</label><input id="f_residence" type="text" value="'+esc(cmd.residence)+'" placeholder="Cotonou, Fidjrossè">'+repique+'</div>'+
      '<div class="fd full"><label for="f_mail">Email <span style="font-weight:500;text-transform:none">(facultatif)</span></label><input id="f_mail" type="email" inputmode="email" autocomplete="email" value="'+esc(cmd.mail)+'" placeholder="vous@gmail.com"></div>'+
    ...
    '</div>'+
    /* LES DEUX MESSAGES QU'HILLARY S'ENGAGE À ENVOYER. C'est sa phrase, et
       c'est ce qui rend le numéro obligatoire : le dire ici vaut mieux que
       de laisser la cliente se demander pourquoi on le lui réclame. */
    '<div class="momo"><svg viewBox="0 0 24 24"><path d="M4 5.5h16v11H9.5L5.5 20v-3.5H4z"/><path d="M8.5 9.5h7M8.5 12.5h4.5"/></svg>'+
    '<span><strong>Vous serez prévenue deux fois.</strong> Un message dès que votre commande '+
    'est validée, puis un message ou un appel dès que votre tenue est prête. C\'est ce numéro '+
    'que l\'atelier utilisera.</span></div>'+
    '<div class="momo"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M9 10h6M9 14h6M12 7v10"/></svg>'+
...
function vueEnvoi(){
  return '<div class="fin">'+
    ...
    /* LES DEUX MESSAGES D'HILLARY, redits à l'endroit où la cliente quitte le
       site. C'est le seul moment où elle se demande « et maintenant ? ». */
    '<p class="suite"><b>Ensuite :</b> vous recevez un message dès que votre commande '+
    'est validée, puis un message ou un appel dès que votre tenue est prête.</p>'+
    recapCommande()+
...
function recapCommande(){
  ...
  if(cmd.mode === "expedition"){
    var f = fraisLivraison();
    h += '<div class="li"><span>Expédition '+esc(cmd.pays?cmd.pays.nom:"")+
         (cmd.ville.trim() ? ", "+esc(cmd.ville.trim()) : "")+'</span><span>'+
...
function peutAvancer(){
  if(cmd.actif){
    if(cmd.etape === 1){
      ...
      /* la ville avec le pays : sans elle, « Côte d'Ivoire » serait toute
         l'adresse de livraison */
      if(cmd.mode === "expedition") return !!cmd.pays && !!cmd.ville.trim();
      return false;
    }
    /* ⚠️ LE NUMÉRO NE PEUT PLUS ÊTRE REMPLACÉ PAR UN EMAIL. Hillary annonce
       qu'elle enverra un message OU qu'elle appellera : sans numéro elle ne
       peut faire ni l'un ni l'autre. Ça n'exclut personne — qui n'a pas
       WhatsApp a un téléphone, et c'est précisément lui qu'on appellera.
       L'email reste offert, en plus, jamais à la place. */
    if(cmd.etape === 2) return manqueCoord().length === 0;
    return true;
  }
  var p = etat.piece;
  if(etat.etape === 1){
...
function brancher(){
  ...
  lieCmd("f_ville","ville"); lieCmd("f_repere","repere");
  lieCmd("f_prenom","prenom"); lieCmd("f_nom","nom");
  lieCmd("f_tel","tel"); lieCmd("f_residence","residence");
  lieCmd("f_mail","mail"); lieCmd("f_note","note");

  var mm = document.getElementById("btMemeLieu");
  if(mm) mm.addEventListener("click", function(){
    cmd.residence = cmd.ville.trim();
    dessiner();
  });
...
function majSuiv(){
  ...
  /* ⚠️ ELLE SE MET À JOUR À LA FRAPPE, PAS AU REDESSIN. `dessiner()` refait
     tout le corps de la modale : l'appeler à chaque touche ferait perdre le
     curseur et le clavier du téléphone. La ligne se réécrit toute seule. */
  var e = document.getElementById("err");
  if(e && cmd.actif){
    var dit = commenceEtape() && manqueEtape().length ? texteManque() : "";
    e.textContent = dit;
    e.classList.toggle("on", !!dit);
  }
}
...
function message(){
  ...
  if(cmd.mode === "retrait"){
    ...
    if(cmd.repere.trim()) L.push("Repère : "+cmd.repere.trim());
  ...
  /* ⚠️ LES CINQ INFORMATIONS QU'HILLARY A DEMANDÉES, chacune sur sa ligne
     et sous son nom. Elle les recopie dans son carnet : un « Prénom Nom »
     collé oblige à deviner où l'un finit, et la ville de livraison n'est pas
     le lieu de résidence. Les quatre premières sont obligatoires, donc
     toujours présentes ; on ne teste que les facultatives. */
  L.push("");
  L.push("*CLIENT*");
  L.push("Nom : "+cmd.nom.trim());
  L.push("Prénom : "+cmd.prenom.trim());
  L.push("Téléphone : "+cmd.tel.trim());
  L.push("Lieu de résidence : "+cmd.residence.trim());
