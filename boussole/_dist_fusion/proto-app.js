    ...
    function greetPool() {   // le MIROIR : phrases construites sur TES chiffres du moment
      ...
      try {
        ...
        // --- pilotage : les chiffres que peu de commerçants connaissent
        const pm = pointMort(), tr2 = treso(), sc2 = scoreSante(), pj2 = projectionMois();
        add('d-score', 'ton commerce est à <b>' + sc2.total + '/100</b> — ' + sc2.lbl.toLowerCase() + '. ' + sc2.why, 3.5);
        if (pm.caNecessaire > 0 && !pm.atteintLe) add('d-pm', 'ton <b>point mort</b> du mois est de ' + fF(pm.caNecessaire) + ' : il te reste <b>' + fF(pm.reste) + '</b> à vendre avant de commencer à gagner. Soit ' + fF(pm.vital) + ' par jour ouvré.');
        if (pm.atteintLe) add('d-pmok', 'tu as passé ton point mort le <b>' + fDate(pm.atteintLe) + '</b> : depuis, chaque vente va directement dans ta poche.');
        if (tr2.aCredit > 0) add('d-cash', 'attention : <b>' + fF(tr2.aCredit) + '</b> de tes ventes du mois sont à crédit. C\'est dans ton bénéfice, mais pas encore dans ta caisse.');
        if (tr2.autonomie !== null && tr2.autonomie < 10 && tr2.sorties > 0) add('d-auto', 'avec ta caisse actuelle, tu tiens <b>' + tr2.autonomie + ' jours</b> de charges. En dessous de 15, on ne dort pas tranquille.');
        if (pj2.caMois > 0 && pj2.restant > 0) add('d-proj', 'à ce rythme, tu finiras le mois autour de <b>' + fF(pj2.fin) + '</b>' + (pj2.manque > 0 ? ', soit ' + fF(pj2.manque) + ' sous ton objectif. Il faudrait ' + fF(pj2.parJourRequis) + ' par jour.' : '. Tu tiens ton objectif.'));
        const ps2 = produitsStats(30);
        if (ps2.list.length) { const c = ps2.list[0]; add('d-champ', '<b>' + c.nom + '</b> t\'a rapporté ' + fF(c.marge) + ' de bénéfice en 30 jours. C\'est ton champion — ne tombe jamais en rupture dessus.'); }
        if (ps2.dormants.length >= 2) add('d-dorm', '<b>' + fF(ps2.dormants.reduce((a, d) => a + d.immo, 0)) + '</b> dorment dans ' + ps2.dormants.length + ' produits que personne n\'achète. Cet argent-là pourrait travailler.');
        const vsm = ventesIn(t0m, Date.now() + 1);
        if (vsm.length >= 3) add('d-panier', 'ton panier moyen ce mois : <b>' + fF(Math.round(vsm.reduce((a, v) => a + v.total, 0) / vsm.length)) + '</b>. Le faire monter de 10%, c\'est souvent plus facile que de trouver 10% de clients en plus.', 2);
        const sv2 = stockValeur();
        if (!isService() && sv2.cout > 0 && sv2.rotation < 0.8) add('d-rota', 'ton stock ne tourne que <b>' + sv2.rotation.toFixed(1) + ' fois par mois</b> : ' + fF(sv2.cout) + ' immobilisés qui dorment. Achète moins, mais plus souvent.');
        const f2 = fiabilite();
        if (f2.jours >= 7 && f2.pct < 60) add('d-fiab', 'tu n\'as noté que <b>' + f2.saisis + ' jours sur ' + f2.jours + '</b> ce mois-ci. Mes calculs valent ce que valent tes saisies — note chaque jour, même vite fait.');
      } catch (er) {}
    ...
    function accueilHTML(cls) {
      ...
      return '<div class="screen home portal" style="--sc:var(--acc)">' +   // portail « retour au nexus » (clip-path circulaire) au lieu de la caméra cine/hud
          ...
          (masked ? '' : (function () { const a = prochaineAction(); return '<button class="todo" data-goto="' + a.nav + '"><span class="todo__k">&#9889; À faire maintenant</span>' +
            '<span class="todo__row"><span class="todo__ic">' + a.ic + '</span><span class="todo__tx"><b>' + a.t + '</b><span>' + a.s + '</span></span><span class="todo__go">&rsaquo;</span></span></button>'; })()) +
    ...
    /* ---- Le rythme du commerce : panier moyen, tickets, jours forts, heures de pointe ---- */
    function panierHTML(r) {
      const vs = ventesIn(r.t0, Math.min(r.t1, Date.now() + 1));
      if (!vs.length) return '';
      const ca = vs.reduce((s, v) => s + (+v.total || 0), 0);
      const panier = Math.round(ca / vs.length);
      const arts = vs.reduce((s, v) => s + (v.items || []).reduce((s2, it) => s2 + it.qty, 0), 0);
      const parVente = (arts / vs.length);
      // comparaison du panier avec la période précédente de même durée
      let evo = '';
      const span = r.t1 - r.t0;
      if (isFinite(span)) {
        const pv = ventesIn(r.t0 - span, r.t0);
        if (pv.length) {
          const pp = pv.reduce((s, v) => s + v.total, 0) / pv.length;
          const d = Math.round((panier - pp) / pp * 100);
          evo = d === 0 ? 'stable' : (d > 0 ? '+' + d + '% vs avant' : d + '% vs avant');
        }
      }
      return '<div class="panel"><h3 class="panel__ttl" style="--sc:var(--acc2)"><i></i>Le rythme de tes ventes</h3>' +
        '<div class="tres" style="margin-top:2px">' +
          '<div class="tcard"><span>Panier moyen</span><b>' + fF(panier) + '</b><small>' + (evo || 'par vente') + '</small></div>' +
          '<div class="tcard"><span>Nombre de ventes</span><b>' + vs.length + '</b><small>' + parVente.toFixed(1) + ' article' + (parVente >= 2 ? 's' : '') + ' par vente</small></div>' +
        '</div>' +
        '<p class="ia__txt" style="margin-top:10px">Pour gagner plus, deux leviers : <b>servir plus de clients</b> ou <b>faire monter le panier</b> (proposer l\'article qui va avec).</p></div>';
    }
    function rythmeHTML(r) {
      const vs = ventesIn(r.t0, Math.min(r.t1, Date.now() + 1));
      if (vs.length < 4) return '';
      const J = ['Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam'], parJ = [0, 0, 0, 0, 0, 0, 0], parH = {};
      vs.forEach((v) => { const d = new Date(v.ts); parJ[d.getDay()] += (+v.total || 0); const h = d.getHours(); parH[h] = (parH[h] || 0) + (+v.total || 0); });
      const ordre = [1, 2, 3, 4, 5, 6, 0];   // semaine qui commence lundi
      const mxJ = Math.max.apply(null, parJ) || 1;
      const bestJ = ordre.reduce((a, b) => parJ[b] > parJ[a] ? b : a, ordre[0]);
      const barsJ = ordre.map((k, i) => '<div class="rbar' + (k === bestJ && parJ[k] > 0 ? ' rbar--best' : '') + '" style="--i:' + i + '"><span class="rbar__b" style="height:' + Math.max(3, parJ[k] / mxJ * 100).toFixed(0) + '%"></span><span class="rbar__l">' + J[k] + '</span></div>').join('');
      const hs = Object.keys(parH).map(Number).sort((a, b) => a - b);
      let heureHTML = '';
      if (hs.length >= 3) {
        const h0 = Math.max(5, hs[0] - 1), h1 = Math.min(23, hs[hs.length - 1] + 1);
        const mxH = Math.max.apply(null, hs.map((h) => parH[h])) || 1;
      ...
      return '<div class="panel"><h3 class="panel__ttl" style="--sc:var(--acc2)"><i></i>Tes jours forts</h3><div class="rythm">' + barsJ + '</div>' +
        '<p class="ia__txt">Ton meilleur jour : <b>' + ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'][bestJ] + '</b> (' + fF(parJ[bestJ]) + '). ' +
        'Ton jour le plus faible mérite une promo — ou du repos.</p></div>' + heureHTML;
    }
    function comparaisonHTML(r) {   // la période choisie vs la précédente de même durée
      const span = r.t1 - r.t0;
      if (!isFinite(span) || r.t0 === 0) return '';
      const caA = caIn(r.t0, r.t1), caB = caIn(r.t0 - span, r.t0);
      const benA = caA - depTotIn(r.t0, r.t1), benB = caB - depTotIn(r.t0 - span, r.t0);
      if (caA === 0 && caB === 0) return '';
      ...
      return '<div class="panel"><h3 class="panel__ttl" style="--sc:var(--acc2)"><i></i>Comparé à la période d\'avant</h3>' +
        ...
        '<p class="ia__txt" style="margin-top:10px">' +
          (pct > 2 ? 'Tu progresses. ' : pct < -2 ? 'Tu recules sur cette période. ' : 'Tu es stable. ') +
          'Côté bénéfice : <b>' + (benA >= 0 ? '+' : '') + fF(benA) + '</b> contre ' + (benB >= 0 ? '+' : '') + fF(benB) + ' avant.' +
        '</p></div>';
    }
    function statsHTML() {
      const r = statRange();
...
>>>>>>> apport
    ...
    /* ===== ÉCRAN « MES PRODUITS » : lequel te rapporte VRAIMENT (marge × volume) ===== */
    let _prodPer = 30;
    function produitsHTML() {
      ...
      const mxMarge = Math.max.apply(null, ps.list.map((p) => p.marge)) || 1;
      const rows = ps.list.slice(0, 12).map((p, i) => {
        const pct = ps.caTot > 0 ? Math.round(p.ca / ps.caTot * 100) : 0;
        const mu = p.qty > 0 ? Math.round(p.marge / p.qty) : 0;
        return '<div class="prow' + (p.vital ? ' prow--vital' : '') + '" style="--i:' + i + '">' +
          '<span class="prow__n">' + (i + 1) + '</span>' +
          '<span class="prow__id"><span class="prow__nm">' + p.nom + '</span>' +
            '<span class="prow__sub">' + p.qty + ' vendu' + (p.qty > 1 ? 's' : '') + ' · ' + fF(p.ca) + ' de chiffre (' + pct + '%) · ' + fF(mu) + ' de marge l\'unité</span>' +
            '<span class="pbar"><span style="width:' + Math.max(4, p.marge / mxMarge * 100).toFixed(0) + '%"></span></span></span>' +
          '<span class="prow__amt"><b>' + fF(p.marge) + '</b><small>rapporté</small></span></div>';
      }).join('');
      const vitals = ps.list.filter((p) => p.vital);
      const champion = ps.list[0], champCA = ps.list.slice().sort((a, b) => b.ca - a.ca)[0];
      const alerte = champion && champCA && champion.nom !== champCA.nom
        ? '<div class="ia ia--warn"><span class="ia__orb"></span><span class="ia__chip">&#10022; À savoir</span><h3 class="ia__title">Ton best-seller n\'est pas ton meilleur produit</h3>' +
          '<p class="ia__txt"><b>' + champCA.nom + '</b> fait le plus de chiffre (' + fF(champCA.ca) + '), mais c\'est <b>' + champion.nom + '</b> qui te rapporte le plus (' + fF(champion.marge) + ' de bénéfice). Mets <b>' + champion.nom + '</b> en avant : même effort, plus d\'argent dans ta poche.</p></div>'
        : '<div class="ia ia--good"><span class="ia__orb"></span><span class="ia__chip">&#10022; Ton champion</span><h3 class="ia__title">' + champion.nom + '</h3>' +
    ...
    /* ============================================================================
       MOTEUR DE PILOTAGE — les chiffres qui font vivre ou mourir un commerce
       (tout est calculé sur les données déjà saisies, aucune saisie en plus)
       ========================================================================== */
    const ventesIn = (t0, t1) => (SM.ventes || []).filter((v) => v.ts >= t0 && v.ts < t1);
    const isService = () => (SM.biz && SM.biz.activite) === 'services';
    // charges FIXES du mois = sorties business hors achat de marchandise (le stock, lui, est un coût variable)
    const chargesMois = () => (SM.depenses || []).filter((d) => !d.perso && d.kind !== 'achat' && d.ts >= startOfMonth()).reduce((s, d) => s + (+d.montant || 0), 0);
    function tauxMarge() {   // marge moyenne réelle sur 60 j (repli : marge théorique du catalogue, puis 40 %)
      const vs = ventesIn(Date.now() - 60 * DAY, Date.now() + 1);
      const ca = vs.reduce((s, v) => s + (+v.total || 0), 0), co = vs.reduce((s, v) => s + (+v.cout || 0), 0);
      if (ca > 0 && co > 0) return Math.max(0.05, Math.min(0.95, (ca - co) / ca));
      const st = SM.stock.filter((p) => p.prix > 0);
      if (st.length) { const m = st.reduce((s, p) => s + (p.prix - (p.cout || 0)) / p.prix, 0) / st.length; if (m > 0) return Math.max(0.05, Math.min(0.95, m)); }
      return 0.4;
    }
    function joursOuverture() {   // jours réellement travaillés (30 j glissants), défaut 26
      const set = {}; ventesIn(Date.now() - 30 * DAY, Date.now() + 1).forEach((v) => { set[dayStart(v.ts)] = 1; });
      const n = Object.keys(set).length;
      return n >= 5 ? Math.min(31, Math.round(n)) : 26;
    }
    function pointMort() {   // LE chiffre : combien il faut vendre pour ne rien perdre
      const charges = chargesMois(), tm = tauxMarge();
      const caNecessaire = tm > 0 ? Math.round(charges / tm) : 0;
      const jo = joursOuverture(), vital = Math.round(caNecessaire / Math.max(1, jo));
      const t0 = startOfMonth(), caMois = caIn(t0, Date.now() + 1);
      let atteintLe = 0, cum = 0;   // le jour du mois où le point mort a été franchi
      const vs = ventesIn(t0, Date.now() + 1).slice().sort((a, b) => a.ts - b.ts);
      for (let i = 0; i < vs.length; i++) { cum += (+vs[i].total || 0); if (caNecessaire > 0 && cum >= caNecessaire) { atteintLe = vs[i].ts; break; } }
      return { charges, tm, caNecessaire, jours: jo, vital, caMois, atteintLe, pct: caNecessaire > 0 ? caMois / caNecessaire * 100 : (caMois > 0 ? 100 : 0), reste: Math.max(0, caNecessaire - caMois) };
    }
    function treso() {   // bénéfice ≠ argent en poche : ce qui est VRAIMENT rentré
      const t0 = startOfMonth();
      const vs = ventesIn(t0, Date.now() + 1);
      const encaisse = vs.filter((v) => v.mode !== 'credit').reduce((s, v) => s + (+v.total || 0), 0);
      const aCredit = vs.filter((v) => v.mode === 'credit').reduce((s, v) => s + (+v.total || 0), 0);
      const dehors = (SM.clients || []).reduce((s, c) => s + (+c.dette || 0), 0);
      const sorties = (SM.depenses || []).filter((d) => d.ts >= t0).reduce((s, d) => s + (+d.montant || 0), 0);   // perso compris : ça sort vraiment
      const cash = encaisse - sorties;
      const chargesJour = chargesMois() / Math.max(1, new Date().getDate());
      const autonomie = chargesJour > 0 ? Math.max(0, Math.floor(cash / chargesJour)) : null;
      return { encaisse, aCredit, dehors, sorties, cash, autonomie };
    }
    function fiabilite() {   // sans saisie régulière, les statistiques mentent : on le dit
      const t0 = startOfMonth(), jours = new Date().getDate(), set = {};
      (SM.ventes || []).forEach((v) => { if (v.ts >= t0) set[dayStart(v.ts)] = 1; });
      (SM.depenses || []).forEach((d) => { if (d.ts >= t0) set[dayStart(d.ts)] = 1; });
      const saisis = Object.keys(set).length;
      return { saisis, jours, pct: jours > 0 ? Math.round(saisis / jours * 100) : 0 };
    }
    function stockValeur() {   // l'argent qui dort sur l'étagère + vitesse de rotation
      const phys = SM.stock.filter((p) => p.type === 'physique');
      const cout = phys.reduce((s, p) => s + (p.cout || 0) * (p.qty || 0), 0);
      const vente = phys.reduce((s, p) => s + (p.prix || 0) * (p.qty || 0), 0);
      const coutVendu30 = ventesIn(Date.now() - 30 * DAY, Date.now() + 1).reduce((s, v) => s + (+v.cout || 0), 0);
      const rotation = cout > 0 ? coutVendu30 / cout : 0;
      return { cout, vente, rotation, dort: vente - cout };
    }
    function produitsStats(jours) {   // par produit : combien il rapporte VRAIMENT (marge × volume)
      const t0 = Date.now() - (jours || 30) * DAY, m = {};
      ventesIn(t0, Date.now() + 1).forEach((v) => {
        (v.items || []).forEach((it) => {
          const p = SM.stock.find((x) => x.nom === it.nom);
          const cu = p ? (p.cout || 0) : Math.round(it.prix * 0.55);
          const e = m[it.nom] || (m[it.nom] = { nom: it.nom, qty: 0, ca: 0, marge: 0, last: 0, ref: p || null });
          e.qty += it.qty; e.ca += it.prix * it.qty; e.marge += (it.prix - cu) * it.qty; e.last = Math.max(e.last, v.ts);
        });
      });
      const list = Object.keys(m).map((k) => m[k]);
      const caTot = list.reduce((s, e) => s + e.ca, 0);
      list.sort((a, b) => b.marge - a.marge);
      let cum = 0;   // règle 20/80 : les produits qui font 80 % du chiffre
      list.slice().sort((a, b) => b.ca - a.ca).forEach((e) => { cum += e.ca; e.vital = caTot > 0 && (cum - e.ca) < caTot * 0.8; });
      const dormants = SM.stock.filter((p) => p.type === 'physique' && (p.qty || 0) > 0 && !m[p.nom])
        .map((p) => ({ nom: p.nom, qty: p.qty, immo: (p.cout || 0) * p.qty }));
      return { list, caTot, dormants, jours: jours || 30 };
    }
    function recurrentMois() {   // revenu récurrent (abonnements) : le chiffre qui rassure
      const noms = SM.stock.filter((p) => p.recurrent).map((p) => p.nom);
      if (!noms.length) return 0;
      return ventesIn(startOfMonth(), Date.now() + 1).reduce((s, v) => s + (v.items || []).reduce((s2, it) => noms.indexOf(it.nom) >= 0 ? s2 + it.prix * it.qty : s2, 0), 0);
    }
    function projectionMois() {   // « à ce rythme, tu finiras à… »
      const now = new Date(), jourDuMois = now.getDate();
      const dansLeMois = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
      const caMois = caIn(startOfMonth(), Date.now() + 1);
      const parJour = caMois / Math.max(1, jourDuMois);
      const fin = Math.round(parJour * dansLeMois);
      const obj = (SM.objectifs && SM.objectifs.mois) || 0;
      return { caMois, parJour: Math.round(parJour), fin, restant: dansLeMois - jourDuMois, obj, manque: obj > 0 ? Math.max(0, obj - fin) : 0, parJourRequis: obj > 0 && dansLeMois > jourDuMois ? Math.round(Math.max(0, obj - caMois) / (dansLeMois - jourDuMois)) : 0 };
    }
    function scoreSante() {   // 5 notes -> un seul chiffre compréhensible en 2 secondes
      const e = enveloppes(), t = treso(), pm = pointMort(), f = fiabilite(), parts = [];
      // 1. Rentabilité : la marge du mois couvre-t-elle les charges ?
      let r = 0;
      if (e.ca > 0) r = e.manque > 0 ? Math.max(0, 20 * (1 - e.manque / Math.max(1, e.charges))) : (e.env3 > 0 ? 20 : 14);
      parts.push({ k: 'Rentabilité', v: r, max: 20, bad: 'ta marge ne couvre pas encore tes charges' });
      // 2. Trésorerie : combien de jours de charges tiens-tu ?
      const aut = t.autonomie === null ? 30 : t.autonomie;
      const tr = Math.max(0, Math.min(20, aut / 30 * 20));
      parts.push({ k: 'Trésorerie', v: tr, max: 20, bad: 'ta caisse ne tient que ' + aut + ' jour' + (aut > 1 ? 's' : '') + ' de charges' });
      // 3. Argent dehors : part des créances dans le CA du mois
      const ratio = e.ca > 0 ? t.dehors / e.ca : 0;
      const cr = Math.max(0, Math.min(20, 20 * (1 - ratio / 0.4)));
      parts.push({ k: 'Crédits clients', v: e.ca > 0 ? cr : 20, max: 20, bad: fF(t.dehors) + ' dorment chez tes clients' });
      // 4. Stock (ou régularité des ventes pour les services)
      let sk = 20, badSk = '';
      if (!isService()) {
        const phys = SM.stock.filter((p) => p.type === 'physique');
        if (phys.length) {
          const rupt = phys.filter((p) => (p.qty || 0) <= 0).length;
          const dor = produitsStats(30).dormants.length;
          sk = Math.max(0, 20 - rupt * 4 - dor * 2);
          badSk = rupt ? rupt + ' produit' + (rupt > 1 ? 's' : '') + ' en rupture' : dor ? dor + ' produit' + (dor > 1 ? 's' : '') + ' qui ne se vend' + (dor > 1 ? 'ent' : '') + ' plus' : '';
        }
      }
      parts.push({ k: isService() ? 'Activité' : 'Stock', v: sk, max: 20, bad: badSk || 'ton stock demande de l\'attention' });
      // 5. Régularité de saisie : sans elle, tous les chiffres mentent
      parts.push({ k: 'Suivi', v: Math.max(0, Math.min(20, f.pct / 100 * 20)), max: 20, bad: 'tu n\'as noté que ' + f.saisis + ' jour' + (f.saisis > 1 ? 's' : '') + ' sur ' + f.jours });
      const total = Math.round(parts.reduce((s, p) => s + p.v, 0));
      const faible = parts.slice().sort((a, b) => (a.v / a.max) - (b.v / b.max))[0];
      const lbl = total >= 80 ? 'Commerce solide' : total >= 60 ? 'Ça tient la route' : total >= 40 ? 'Fragile, mais rattrapable' : 'Danger : il faut agir';
      const col = total >= 75 ? 'var(--good)' : total >= 50 ? 'var(--acc2)' : 'var(--bad)';
      return { total, parts, faible, lbl, col, why: total >= 85 ? 'Continue exactement comme ça.' : 'Ce qui te pénalise le plus : ' + faible.bad + '.' };
    }
...
=======
      ...
      const scoreHTML = '<div class="score"><div class="score__wrap">' +
          '<svg class="score__svg" viewBox="0 0 132 132"><circle class="score__track" cx="66" cy="66" r="' + R + '"/>' +
          '<circle class="score__val" cx="66" cy="66" r="' + R + '" stroke="' + sc.col + '" stroke-dasharray="' + C.toFixed(1) + '" stroke-dashoffset="' + C.toFixed(1) + '" data-scoff="' + (C * (1 - sc.total / 100)).toFixed(1) + '" style="filter:drop-shadow(0 0 7px ' + sc.col + ')"/></svg>' +
          '<div class="score__mid"><b class="score__num" style="color:' + sc.col + '">' + sc.total + '</b><span class="score__cent">sur 100</span></div>' +
        '</div><b class="score__lbl">' + sc.lbl + '</b><span class="score__why">' + sc.why + '</span>' +
        '<div class="score__parts">' + sc.parts.map((p) => { const r = p.v / p.max; return '<span class="spart spart--' + (r >= 0.75 ? 'ok' : r >= 0.45 ? 'mid' : 'bad') + '"><i></i>' + p.k + ' ' + Math.round(p.v) + '/' + p.max + '</span>'; }).join('') + '</div></div>';
      // jauge du point mort : le seuil de survie du mois
      const pmPct = Math.min(100, pm.pct), markAt = pm.caNecessaire > 0 ? Math.min(97, 100 * pm.caNecessaire / Math.max(pm.caNecessaire, pm.caMois) ) : 0;
      const pmHTML = pm.caNecessaire > 0 ? '<div class="bem"><div class="bem__top"><span class="bem__ttl">Ton point mort du mois</span><b class="bem__amt">' + fF(pm.caNecessaire) + '</b></div>' +
          '<div class="bem__track"><span class="bem__fill" data-bemfill="' + pmPct.toFixed(0) + '"></span>' + (pm.caMois > pm.caNecessaire ? '<span class="bem__mark" style="left:' + markAt.toFixed(0) + '%"></span>' : '') + '</div>' +
          '<p class="bem__txt">' + (pm.atteintLe
            ? 'Tu as passé le point mort <b>le ' + fDate(pm.atteintLe) + '</b> : depuis, chaque vente est du <b>bénéfice</b>.'
            : 'Il te reste <b>' + fF(pm.reste) + '</b> à vendre ce mois-ci pour ne rien perdre.') +
          ' Avec ' + fF(pm.charges) + ' de charges et une marge de <b>' + Math.round(pm.tm * 100) + '%</b>, il te faut <b>' + fF(pm.vital) + ' par jour ouvré</b>.</p></div>'
        : '';
      // trésorerie : ce qui est vraiment rentré, ce qui est encore dehors
      const tresHTML = '<div class="tres">' +
          '<div class="tcard tcard--cash"><span>Argent en caisse</span><b>' + fF(tr.cash) + '</b><small>' + (tr.autonomie !== null ? 'tient ' + tr.autonomie + ' jour' + (tr.autonomie > 1 ? 's' : '') + ' de charges' : 'encaissé moins sorties') + '</small></div>' +
          '<div class="tcard tcard--out"><span>Argent dehors</span><b>' + fF(tr.dehors) + '</b><small>' + (tr.dehors > 0 ? 'chez tes clients à crédit' : 'personne ne te doit rien') + '</small></div>' +
        '</div>' +
        (tr.aCredit > 0 ? '<p class="bem__txt" style="max-width:500px;margin:9px auto 0;text-align:left">⚠️ Attention : <b>' + fF(tr.aCredit) + '</b> de tes ventes du mois sont <b>à crédit</b> — c\'est compté dans ton bénéfice mais ce n\'est <b>pas encore dans ta caisse</b>.</p>' : '');
      // projection de fin de mois
      const pjHTML = pj.caMois > 0 && pj.restant > 0 ? '<div class="bem"><div class="bem__top"><span class="bem__ttl">Fin de mois projetée</span><b class="bem__amt">' + fF(pj.fin) + '</b></div>' +
...
>>>>>>> apport
    ...
    function bindBilan() {
      ...
      const bf = view.querySelector('[data-bemfill]');   // la jauge du point mort se remplit
      if (bf) { const w = bf.dataset.bemfill + '%'; if (rm) bf.style.width = w; else requestAnimationFrame(() => requestAnimationFrame(() => { bf.style.width = w; })); }
      const sc = view.querySelector('[data-scoff]');     // l'anneau du score se dessine
      if (sc) { const o = sc.dataset.scoff; if (rm) sc.style.strokeDashoffset = o; else requestAnimationFrame(() => requestAnimationFrame(() => { sc.style.strokeDashoffset = o; })); }
      const rap = view.querySelector('[data-rapport]');
      if (rap) rap.addEventListener('click', () => { playClick(); rapportMois(); });
      const wa = view.querySelector('[data-rapportwa]');
      if (wa) wa.addEventListener('click', () => { playClick(); partagerBilanWA(); });
    }
    /* ---- Rapport mensuel : la preuve à sortir de l'app (banque, associé, microcrédit) ---- */
    function rapportMois() {
      const e = enveloppes(), pm = pointMort(), tr = treso(), sc = scoreSante(), fi = fiabilite(), sv = stockValeur(), ps = produitsStats(30);
      const mois = MOIS_LBL[new Date().getMonth()] + ' ' + new Date().getFullYear();
      const cats = depByCatIn(startOfMonth(), Date.now() + 1);
      const ligne = (a, b, fort) => '<tr><td>' + a + '</td><td class="r"' + (fort ? ' style="font-weight:800"' : '') + '>' + b + '</td></tr>';
      const css = 'body{font-family:Arial,Helvetica,sans-serif;color:var(--ink);max-width:660px;margin:24px auto;padding:0 20px}h1{font-size:22px;margin:0}h2{font-size:13px;text-transform:uppercase;letter-spacing:.08em;color:#666;margin:22px 0 6px;border-bottom:1px solid #ddd;padding-bottom:5px}.h{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid var(--ink);padding-bottom:12px}.sc{text-align:center;border:2px solid var(--ink);border-radius:12px;padding:8px 14px}.sc b{display:block;font-size:26px}.sc span{font-size:10px;text-transform:uppercase;color:#666}table{width:100%;border-collapse:collapse;font-size:13px}td{padding:6px 4px;border-bottom:1px solid #eee}.r{text-align:right;font-variant-numeric:tabular-nums}.note{font-size:12px;color:#555;line-height:1.5;margin-top:8px}.foot{margin-top:26px;font-size:11px;color:#888;text-align:center;border-top:1px solid #ddd;padding-top:10px}';
      const html = '<!doctype html><html><head><meta charset="utf-8"><title>Bilan ' + mois + '</title><style>' + css + '</style></head><body>' +
        '<div class="h"><div><h1>' + bizNom() + '</h1><div style="font-size:12px;color:#666">' + bizLine() + '</div><div style="font-size:13px;margin-top:8px"><b>Bilan de ' + mois + '</b></div></div>' +
        '<div class="sc"><b>' + sc.total + '</b><span>santé /100</span></div></div>' +
        '<h2>L\'essentiel</h2><table>' +
        ligne('Ventes encaissées (chiffre d\'affaires)', fF(e.ca), 1) +
        ligne('Coût de revient des ventes', fF(e.cout)) +
        ligne('Marge dégagée', fF(e.marge)) +
        ligne('Charges du mois', fF(e.charges)) +
        ligne('<b>Bénéfice net</b>', '<b>' + fF(e.env3 - (e.manque > 0 ? e.manque : 0)) + '</b>', 1) +
        '</table>' +
        '<h2>Trésorerie</h2><table>' +
        ligne('Réellement encaissé (espèces + Mobile Money)', fF(tr.encaisse)) +
        ligne('Vendu à crédit (pas encore encaissé)', fF(tr.aCredit)) +
        ligne('Total dû par les clients', fF(tr.dehors)) +
        ligne('Sorties du mois', fF(tr.sorties)) +
        ligne('<b>Argent en caisse</b>', '<b>' + fF(tr.cash) + '</b>', 1) +
        '</table>' +
        '<h2>Seuil de rentabilité</h2><table>' +
        ligne('Point mort du mois (ventes minimum)', fF(pm.caNecessaire)) +
        ligne('Objectif par jour ouvré', fF(pm.vital)) +
        ligne('Taux de marge moyen', Math.round(pm.tm * 100) + ' %') +
        ligne('Statut', pm.atteintLe ? 'Point mort franchi le ' + fDate(pm.atteintLe) : 'Reste ' + fF(pm.reste) + ' à vendre', 1) +
        '</table>' +
        (cats.length ? '<h2>Où part l\'argent</h2><table>' + cats.map((c) => ligne(c.name, fF(c.val) + ' (' + Math.round(c.val / Math.max(1, e.charges + e.cout) * 100) + ' %)')).join('') + '</table>' : '') +
        (ps.list.length ? '<h2>Meilleurs produits (30 jours, par bénéfice apporté)</h2><table>' + ps.list.slice(0, 6).map((p, i) => ligne((i + 1) + '. ' + p.nom + ' <span style="color:#888">(' + p.qty + ' vendus)</span>', fF(p.marge))).join('') + '</table>' : '') +
        (!isService() && sv.cout > 0 ? '<h2>Stock</h2><table>' + ligne('Valeur immobilisée (au coût)', fF(sv.cout)) + ligne('Valeur de revente', fF(sv.vente)) + ligne('Rotation mensuelle', sv.rotation.toFixed(1) + ' fois') + '</table>' : '') +
        '<p class="note"><b>Fiabilité des données :</b> saisies effectuées ' + fi.saisis + ' jour(s) sur ' + fi.jours + ' (' + fi.pct + ' %). ' + sc.why + '</p>' +
        '<div class="foot">Document généré par Boussole — NEBULA Agency' + (bizLegal() ? ' · ' + bizLegal() : '') + '</div>' +
        '<scr' + 'ipt>window.onload=function(){window.print()}</scr' + 'ipt></body></html>';
      const w = window.open('', '_blank'); if (w) { w.document.write(html); w.document.close(); }
      toast('Rapport de ' + mois + ' prêt à imprimer ou enregistrer en PDF', { tone: 'good' });
    }
    function partagerBilanWA() {
      const e = enveloppes(), pm = pointMort(), tr = treso(), sc = scoreSante();
      const mois = MOIS_LBL[new Date().getMonth()] + ' ' + new Date().getFullYear();
      const msg = '*' + bizNom() + '* — bilan de ' + mois + '\n' +
        '\nSanté du commerce : *' + sc.total + '/100* (' + sc.lbl + ')' +
        '\nVentes : *' + fF(e.ca) + '*' +
        '\nCharges : ' + fF(e.charges) +
        '\nBénéfice net : *' + fF(e.env3) + '*' +
        '\nArgent en caisse : ' + fF(tr.cash) +
        (tr.dehors > 0 ? '\nÀ récupérer chez les clients : ' + fF(tr.dehors) : '') +
        '\nPoint mort : ' + fF(pm.caNecessaire) + (pm.atteintLe ? ' (franchi le ' + fDate(pm.atteintLe) + ')' : ' — reste ' + fF(pm.reste)) +
        '\n\n_Généré par Boussole_';
...
=======
      ...
      if (pm.atteintLe) A.push({ tone: 'good', ac: 'var(--good)', ic: ICO_STAR, nav: 'bilan', tx: 'Point mort franchi <b>le ' + fDate(pm.atteintLe) + '</b> : tout ce que tu vends maintenant est du <b>bénéfice</b>.' });
...
>>>>>>> apport
    ...
    function answerLocal(q) {
      ...
      if (/point mort|seuil|rentab.*seuil|combien.*pour ne rien perdre/.test(s)) { const pm = pointMort(); return pm.caNecessaire > 0 ? 'Ton point mort du mois est de <b>' + fF(pm.caNecessaire) + '</b> (charges ' + fF(pm.charges) + ' ÷ marge ' + Math.round(pm.tm * 100) + '%), soit <b>' + fF(pm.vital) + ' par jour ouvré</b>. ' + (pm.atteintLe ? 'Tu l\'as franchi le ' + fDate(pm.atteintLe) + ' : le reste est du bénéfice.' : 'Il te reste ' + fF(pm.reste) + ' à faire.') : 'Note d\'abord tes charges du mois et je te calcule ton point mort.'; }
      if (/tr[ée]sorerie|caisse.*r[ée]elle|combien.*en caisse|cash/.test(s)) { const t = treso(); return 'En caisse : <b>' + fF(t.cash) + '</b> (encaissé ' + fF(t.encaisse) + ' − sorties ' + fF(t.sorties) + ').' + (t.dehors > 0 ? ' En plus, <b>' + fF(t.dehors) + '</b> sont encore chez tes clients.' : '') + (t.autonomie !== null ? ' Ça te fait <b>' + t.autonomie + ' jours</b> de charges devant toi.' : ''); }
      if (/panier|ticket moyen|moyenne.*vente/.test(s)) { const vs = ventesIn(startOfMonth(), Date.now() + 1); if (!vs.length) return 'Aucune vente ce mois-ci pour calculer ton panier moyen.'; const pan = Math.round(vs.reduce((a, v) => a + v.total, 0) / vs.length); return 'Ton panier moyen ce mois : <b>' + fF(pan) + '</b> sur ' + vs.length + ' ventes. Fais-le monter en proposant systématiquement l\'article qui va avec.'; }
      if (/quel jour|meilleur jour de la semaine|jour fort/.test(s)) { const J = ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'], p = [0, 0, 0, 0, 0, 0, 0]; ventesIn(Date.now() - 60 * DAY, Date.now() + 1).forEach((v) => { p[new Date(v.ts).getDay()] += v.total; }); let b = 0; for (let i = 1; i < 7; i++) if (p[i] > p[b]) b = i; return p[b] > 0 ? 'Sur 60 jours, ton meilleur jour est le <b>' + J[b] + '</b> (' + fF(p[b]) + '). Prépare-le : stock, monnaie, personnel.' : 'Pas encore assez de ventes pour dégager un jour fort.'; }
      if (/dort|dormant|invendu|immobilis/.test(s)) { const d = produitsStats(30).dormants; return d.length ? '<b>' + fF(d.reduce((a, x) => a + x.immo, 0)) + '</b> dorment dans ' + d.length + ' produit(s) invendus depuis 30 jours : ' + d.slice(0, 3).map((x) => '<b>' + x.nom + '</b>').join(', ') + '. Promo, lot, ou on arrête d\'en racheter.' : 'Rien ne dort : tous tes produits en stock se vendent.'; }
      if (/rapporte|rentable.*produit|meilleure marge/.test(s)) { const ps = produitsStats(30); if (!ps.list.length) return 'Pas encore de vente sur 30 jours.'; const c = ps.list[0]; return 'Sur 30 jours, c\'est <b>' + c.nom + '</b> qui te rapporte le plus : <b>' + fF(c.marge) + '</b> de bénéfice (' + c.qty + ' vendus). Attention, ce n\'est pas forcément celui qui fait le plus de chiffre.'; }
      if (/fin de mois|projection|je vais finir|combien.*ce mois/.test(s)) { const p = projectionMois(); return p.caMois > 0 ? 'À ton rythme (<b>' + fF(p.parJour) + '/jour</b>), tu finiras le mois autour de <b>' + fF(p.fin) + '</b>.' + (p.manque > 0 ? ' Pour ton objectif il faudrait ' + fF(p.parJourRequis) + ' par jour sur les ' + p.restant + ' jours restants.' : '') : 'Encore aucune vente ce mois-ci.'; }
      if (/score|sant[ée]|comment (je )?vais|[çc]a va mon commerce/.test(s)) { const sc = scoreSante(); return 'Ton commerce est à <b>' + sc.total + '/100</b> — ' + sc.lbl + '. ' + sc.why; }
...
>>>>>>> apport
    ...
    const LOCKED = { patron: [], gerant: ['equipe', 'reglages'], vendeur: ['bilan', 'statistiques', 'objectifs', 'carnet', 'factures', 'equipe', 'depenses', 'boussole', 'reglages', 'produits'] };
...
>>>>>>> apport
    ...
    function bindReglages() {
      ...
      const tr2 = g('[data-tourreplay]'); if (tr2) tr2.addEventListener('click', () => { playClick(); startTour(0); });
      const ao = g('[data-aideopen]'); if (ao) ao.addEventListener('click', ouvrirAide);
    }
    ...
    function renderOnboarding() {
      ...
      const finish = (demo) => {
        ...
        smSave(); playSell();
        ov.remove(); bootApp();
        if (!_noTour) setTimeout(() => startTour(0), 700);   // la visite guidée enchaîne juste après l'inscription
        else toast('Bienvenue ' + (dat.prenom || 'Boss') + ' ! Commence par ajouter tes produits au <b>Catalogue</b>.', { tone: 'good', ms: 6000 });
      };
      paint();
    }
    /* ============================================================================
       COMPRENDRE — 1) visite guidée animée  2) aide de chaque écran
       3) « à faire maintenant ». Objectif : qu'un enfant de 5 ans s'y retrouve.
       ========================================================================== */
    const TOUR = [
      { ico: '&#128075;', ttl: 'Bienvenue dans Boussole', txt: 'En moins d\'une minute, je te montre <b>où toucher</b> et <b>à quoi ça sert</b>. Tu pourras revoir cette visite quand tu veux avec le bouton <b>?</b> en haut.', nav: 'accueil', cible: null },
      { ico: '&#128176;', ttl: 'Les deux gros boutons', txt: 'Tout part d\'ici. <b>VENDRE</b> quand un client achète. <b>DÉPENSER</b> quand de l\'argent sort. C\'est tout ce que tu dois faire au quotidien.', nav: 'accueil', cible: '.masto' },
      { ico: '&#128200;', ttl: 'Tes chiffres du jour', txt: 'En haut : ce qu\'il y a dans ta <b>caisse</b>, ce que tu as fait <b>aujourd\'hui</b>, et où tu en es de ton <b>objectif</b>. Touche-les pour voir le détail.', nav: 'accueil', cible: '.vitals' },
      { ico: '&#128271;', ttl: 'La barre du bas', txt: 'Elle ne bouge jamais. <b>Accueil</b>, <b>Caisse</b>, <b>Bilan</b>, <b>Boussole</b> (moi), et <b>Menu</b> pour tout le reste.', nav: 'accueil', cible: '.tabbar' },
      { ico: '&#128722;', ttl: 'Encaisser une vente', txt: 'Touche les produits vendus, choisis <b>Espèces</b>, <b>Mobile Money</b> ou <b>Crédit</b>, puis <b>Encaisser</b>. Je note tout : le stock baisse tout seul.', nav: 'caisse', cible: '.pos__grid' },
      { ico: '&#129534;', ttl: 'Ta note sur 100', txt: 'Le <b>Bilan</b> te donne une note de santé. Si elle baisse, je te dis <b>exactement pourquoi</b> et quoi corriger.', nav: 'bilan', cible: '.score' },
      { ico: '&#10022;', ttl: 'Pose-moi tes questions', txt: 'Écris-moi comme à un ami : « <b>combien j\'ai vendu ?</b> », « <b>qui me doit ?</b> ». Je réponds avec <b>tes vrais chiffres</b>.', nav: 'boussole', cible: '.chatask' },
      { ico: '&#9776;', ttl: 'Le menu, pour tout le reste', txt: 'Tes <b>produits</b>, tes <b>clients</b>, tes <b>factures</b>, ton <b>équipe</b>, tes <b>réglages</b>. Touche ici quand tu cherches quelque chose.', nav: 'accueil', cible: '.hdr__burger' },
      { ico: '&#127881;', ttl: 'C\'est tout !', txt: 'Commence par une chose : <b>ajoute tes produits</b>, puis encaisse ta première vente. Le reste se remplit tout seul. Le bouton <b>?</b> est toujours là si tu doutes.', nav: 'accueil', cible: null },
    ];
    let _tourStep = 0, _tourEl = null;
    function startTour(from) {
      if (_tourEl) return;
      _tourStep = typeof from === 'number' ? from : 0;
      const ov = document.createElement('div'); ov.className = 'tour'; ov.setAttribute('role', 'dialog'); ov.setAttribute('aria-label', 'Visite guidée');
      ov.innerHTML = '<div class="tour__veil" data-veil></div><div class="tour__ring" data-ring hidden></div><span class="tour__hand" data-hand hidden>&#128072;</span><div class="tour__bub" data-bub></div>';
      document.body.appendChild(ov); _tourEl = ov;
      ov.addEventListener('click', (e) => { if (e.target.dataset.veil !== undefined) return; });
      paintTour();
    }
    function endTour(fini) {
      if (!_tourEl) return;
      _tourEl.remove(); _tourEl = null;
      SM.meta.tourDone = true; smSave();
      const hb = document.querySelector('[data-help]');
      if (fini && hb) { hb.classList.add('pulse'); setTimeout(() => hb.classList.remove('pulse'), 8000); }
      if (fini) toast('Visite terminée. Le bouton <b>?</b> en haut t\'explique chaque écran.', { tone: 'good', ms: 6000 });
    }
    function paintTour() {
      const st = TOUR[_tourStep]; if (!st) return endTour(true);
      const go = () => {
        const ring = _tourEl.querySelector('[data-ring]'), bub = _tourEl.querySelector('[data-bub]'),
              veil = _tourEl.querySelector('[data-veil]'), hand = _tourEl.querySelector('[data-hand]');
        const cible = st.cible ? document.querySelector(st.cible) : null;
        const vh = window.innerHeight, vw = window.innerWidth;
        let arrow = 'none', bubTop, box = null;
        if (cible) {
          const b = cible.getBoundingClientRect();
          const pad = 8;
          const x = Math.max(4, b.left - pad), y = Math.max(4, b.top - pad);
          const w = Math.min(vw - 8, b.width + pad * 2), h = Math.min(vh - 8, b.height + pad * 2);
          ring.hidden = false; ring.style.left = x + 'px'; ring.style.top = y + 'px'; ring.style.width = w + 'px'; ring.style.height = h + 'px';
          // le projecteur : le voile se troue autour de la cible
          veil.style.clipPath = 'polygon(0 0, 100% 0, 100% 100%, 0 100%, 0 ' + y + 'px, ' + (x + w) + 'px ' + y + 'px, ' + (x + w) + 'px ' + (y + h) + 'px, ' + x + 'px ' + (y + h) + 'px, ' + x + 'px ' + y + 'px, 0 ' + y + 'px)';
          hand.hidden = false; hand.style.left = (x + w / 2 - 22) + 'px'; hand.style.top = (y + h + 6) + 'px';
          box = { x: x, y: y, w: w, h: h };
        } else {
          ring.hidden = true; hand.hidden = true; veil.style.clipPath = '';
        }
        const dernier = _tourStep === TOUR.length - 1;
        bub.innerHTML = '<span class="tour__ico">' + st.ico + '</span>' +
          '<h3 class="tour__ttl">' + st.ttl + '</h3><p class="tour__txt">' + st.txt + '</p>' +
          '<div class="tour__foot"><span class="tour__dots">' + TOUR.map((_, i) => '<i class="' + (i === _tourStep ? 'on' : '') + '"></i>').join('') + '</span>' +
          (dernier ? '' : '<button class="tour__skip" data-tskip>Passer</button>') +
          '<button class="tour__next" data-tnext>' + (dernier ? 'C\'est parti !' : 'Suivant') + '</button></div>';
        // placement APRÈS écriture du texte : on connaît enfin la hauteur réelle de la bulle
        const bh = bub.offsetHeight || 220, MARGE = 12, HAUT = 64;
        if (box) {
          if (box.y + box.h + 14 + bh <= vh - MARGE) { arrow = 'up'; bubTop = box.y + box.h + 14; }          // sous la cible
          else if (box.y - 14 - bh >= HAUT) { arrow = 'down'; bubTop = box.y - 14 - bh; }                    // au-dessus
          else { arrow = 'none'; bubTop = Math.max(HAUT, Math.min(vh - bh - MARGE, vh / 2 - bh / 2)); }      // pas la place : au centre
        } else { arrow = 'none'; bubTop = Math.max(HAUT, vh / 2 - bh / 2); }
        bubTop = Math.max(MARGE, Math.min(bubTop, vh - bh - MARGE));   // garantie : toujours entièrement visible
        bub.dataset.arrow = arrow;
        bub.style.top = bubTop + 'px';
        bub.querySelector('[data-tnext]').addEventListener('click', () => { playClick(); _tourStep++; paintTour(); });
        const sk = bub.querySelector('[data-tskip]');
        if (sk) sk.addEventListener('click', () => { playSwoosh(); endTour(false); });
      };
      if (st.nav && st.nav !== _curNav) { setActiveNav(st.nav); showSection(st.nav, false); setTimeout(go, st.nav === 'caisse' ? 900 : 420); }
      else setTimeout(go, 60);
    }
    /* ---- Aide contextuelle : « à quoi sert cet écran ? » ---- */
    const AIDE = {
      accueil: { t: 'L\'accueil', p: [['&#128176;', 'Vendre', 'Un client achète ? Touche ce bouton, il t\'emmène à la caisse.'], ['&#128184;', 'Dépenser', 'De l\'argent sort (transport, stock, loyer) ? Note-le ici, sinon ton bénéfice est faux.'], ['&#128202;', 'Tes 3 chiffres', 'Caisse, ventes du jour, objectif. Touche-les pour le détail.'], ['&#128293;', 'La série', 'Chaque jour où tu atteins ton objectif allume une flamme. Ne la casse pas !']] },
      caisse: { t: 'La caisse', p: [['&#128290;', '1. Touche les produits', 'Chaque touche ajoute 1. Le total monte en bas.'], ['&#128179;', '2. Choisis le paiement', 'Espèces, Mobile Money, ou Crédit (le client paiera plus tard).'], ['&#9989;', '3. Encaisse', 'C\'est enregistré : le stock baisse et le chiffre monte tout seul.'], ['&#128100;', 'Crédit', 'Je te demande QUI c\'est, pour ne jamais oublier qui te doit de l\'argent.']] },
      ventes: { t: 'Tes ventes', p: [['&#128220;', 'La liste', 'Toutes tes ventes, la plus récente en haut.'], ['&#128072;', 'Touche une vente', 'Tu vois le détail, tu peux la <b>modifier</b> ou la <b>supprimer</b>.'], ['&#128269;', 'Chercher', 'Tape un produit, un client ou un montant pour retrouver une vente.']] },
      depenses: { t: 'Tes dépenses', p: [['&#10133;', 'Ajouter', 'Le bouton + en bas à droite. Montant, catégorie, et c\'est noté.'], ['&#128717;', 'Achat ou dépense', '« Achat » = marchandise à revendre. « Dépense » = charge (loyer, transport…).'], ['&#128274;', 'Poche perso', 'Si c\'est pour toi et pas pour le commerce, coche-le : ce ne sera pas compté dans le bénéfice.']] },
      catalogue: { t: 'Ton catalogue', p: [['&#128230;', 'Tes produits', 'Nom, prix de vente, et ce que ça te coûte. La marge se calcule seule.'], ['&#128177;', 'Détail du coût', 'Tu peux dire QUI prend quoi (partenaire, hébergement…) : je te dirai quoi réserver sur chaque vente.'], ['&#128202;', 'Quantité', 'Pour les produits physiques : je préviens quand le stock est bas.']] },
      bilan: { t: 'Le bilan', p: [['&#129534;', 'Ta note sur 100', 'Un seul chiffre pour savoir si ça va. Je te dis toujours ce qui te pénalise.'], ['&#127919;', 'Le point mort', 'Le montant à vendre pour ne rien perdre. En dessous, tu perds de l\'argent.'], ['&#128181;', 'Argent en caisse', 'Ce que tu as VRAIMENT. Différent du bénéfice si des clients te doivent.'], ['&#128230;', 'Les 3 enveloppes', 'Ce qui doit racheter le stock, ce qui paie les charges, et ce qui te reste.']] },
      statistiques: { t: 'Les statistiques', p: [['&#128197;', 'Choisis une période', 'Un jour, un mois, une année, ou entre deux dates.'], ['&#128722;', 'Panier moyen', 'Combien un client dépense en moyenne. Le faire monter est plus simple que trouver des clients.'], ['&#128198;', 'Jours forts', 'Le jour où tu vends le plus. Prépare-le, et anime le jour faible.']] },
      produits: { t: 'Mes produits', p: [['&#127942;', 'Classement', 'Par argent RÉELLEMENT gagné, pas par chiffre d\'affaires.'], ['&#9888;', 'Attention', 'Ton produit le plus vendu n\'est pas toujours celui qui rapporte le plus.'], ['&#128564;', 'Ce qui dort', 'Les produits que personne n\'achète : de l\'argent bloqué sur l\'étagère.']] },
      objectifs: { t: 'Tes objectifs', p: [['&#127919;', 'Objectif du jour', 'Ce que tu veux encaisser. Touche « Modifier l\'objectif » pour l\'ajuster.'], ['&#128176;', 'Projets d\'achat', 'Une moto, un congélateur ? Note le prix, mets de côté, je te dis quand tu y seras.']] },
      carnet: { t: 'Le carnet clients', p: [['&#128100;', 'Tes clients', 'Ceux qui te doivent de l\'argent apparaissent en premier.'], ['&#128241;', 'Relancer', 'Un message poli est déjà écrit : touche, ça part sur WhatsApp.'], ['&#9989;', 'Marquer payé', 'Quand le client règle, touche ce bouton : la dette disparaît.']] },
      factures: { t: 'Factures & devis', p: [['&#128196;', 'Créer', 'Le + en bas. Touche les articles, mets le nom du client, c\'est prêt.'], ['&#128438;', 'Envoyer', 'Partage par WhatsApp ou imprime en PDF.'], ['&#128257;', 'Statut', 'Touche le statut pour passer de brouillon à envoyé, puis payé.']] },
      equipe: { t: 'Ton équipe', p: [['&#128273;', 'Un code par personne', 'Chacun a son code : tu vois qui a vendu quoi.'], ['&#128274;', 'Protection', 'Un vendeur ne voit ni ton bénéfice ni tes réglages.'], ['&#128100;', 'Qui tient la caisse', 'Touche le rond en haut à droite pour changer de personne.']] },
      boussole: { t: 'Boussole, ton copilote', p: [['&#128276;', 'Ce que j\'ai repéré', 'Mes alertes : ruptures, dettes qui traînent, objectif en retard.'], ['&#128172;', 'Pose une question', 'Écris comme à un ami. Je réponds avec tes vrais chiffres.'], ['&#10022;', 'Mon avis', 'Je te dis franchement ce qui va et ce qui ne va pas.']] },
      reglages: { t: 'Les réglages', p: [['&#127978;', 'Ton commerce', 'Nom, téléphone, IFU : ça apparaît sur tes factures.'], ['&#128273;', 'Code patron', 'Protège tes chiffres quand quelqu\'un d\'autre tient la caisse.'], ['&#128190;', 'Sauvegarde', 'Exporte de temps en temps : tes données restent dans ce téléphone.']] },
    };
    function ouvrirAide() {
      const a = AIDE[_curNav] || AIDE.accueil;
      const sheet = document.querySelector('[data-aidesheet]'), scrim = document.querySelector('[data-aideclose]'),
            body = document.querySelector('[data-aidebody]'), ttl = document.querySelector('[data-aidettl]');
      ttl.textContent = a.t + ' — à quoi ça sert ?';
      body.innerHTML = '<div class="aidelist">' + a.p.map((x, i) => '<div class="aideitem" style="--i:' + i + '"><span class="aideitem__ic">' + x[0] + '</span><span class="aideitem__tx"><b>' + x[1] + '</b>' + x[2] + '</span></div>').join('') + '</div>' +
        '<button class="sheet__add" data-aidetour>&#127909; Revoir la visite guidée</button>' +
        '<button class="sheet__keep" data-aideok>J\'ai compris</button>';
      const close = () => { sheet.classList.remove('is-open'); scrim.classList.remove('is-open'); };
      scrim.onclick = close;
      body.querySelector('[data-aideok]').addEventListener('click', () => { playClick(); close(); });
      body.querySelector('[data-aidetour]').addEventListener('click', () => { close(); setTimeout(() => startTour(0), 320); });
      sheet.classList.add('is-open'); scrim.classList.add('is-open'); playClick();
    }
    /* ---- « À faire maintenant » : l'accueil dit toujours quoi faire ---- */
    function prochaineAction() {
      const nbP = SM.stock.length, nbV = (SM.ventes || []).length, nbD = (SM.depenses || []).length;
      if (!nbP) return { ic: '&#128230;', t: 'Ajoute tes premiers produits', s: 'Sans catalogue, la caisse est vide. Mets 2 ou 3 produits avec leur prix : ça prend 1 minute.', nav: 'catalogue' };
      if (!nbV) return { ic: '&#128176;', t: 'Encaisse ta première vente', s: 'Va à la caisse, touche un produit, encaisse. Tout le reste se calcule tout seul.', nav: 'caisse' };
      if (!nbD) return { ic: '&#128184;', t: 'Note ta première dépense', s: 'Transport, stock, loyer… Sans les sorties, ton bénéfice est une illusion.', nav: 'depenses' };
      const rupt = SM.stock.filter((p) => p.type === 'physique' && (p.qty || 0) <= 0);
      if (rupt.length) return { ic: '&#128721;', t: 'Réapprovisionne ' + rupt[0].nom, s: rupt.length + ' produit' + (rupt.length > 1 ? 's sont' : ' est') + ' en rupture : chaque heure sans stock est une vente perdue.', nav: 'catalogue' };
      const due = (SM.clients || []).filter((c) => c.dette > 0 && Date.now() - c.lastBuy > 7 * DAY);
      if (due.length) return { ic: '&#128241;', t: 'Relance ' + due[0].nom, s: 'Il te doit ' + fF(due[0].dette) + ' depuis plus d\'une semaine. Un message poli est déjà prêt.', nav: 'carnet' };
      const st = salesToday(), obj = objJour();
      if (obj > 0 && st < obj) return { ic: '&#127919;', t: 'Encore ' + fF(obj - st) + ' pour ton objectif', s: 'Une ou deux bonnes ventes et c\'est fait. Ouvre la caisse.', nav: 'caisse' };
      const f = fiabilite();
      if (f.jours >= 5 && f.pct < 60) return { ic: '&#128221;', t: 'Note ce que tu as vendu aujourd\'hui', s: 'Tu n\'as saisi que ' + f.saisis + ' jours sur ' + f.jours + ' : mes calculs valent ce que valent tes saisies.', nav: 'caisse' };
      const pm = pointMort();
    ...
    function countUpNums(scope) {
      ...
      (scope || view).querySelectorAll('.kpi__num, .benhero__num').forEach((el, i) => {
        const m = el.textContent.match(/^([^\d]*)([\d\s\u00A0\u202F]*\d)(.*)$/);   // le groupe chiffres FINIT sur un chiffre -> l'espace avant « F » reste dans le suffixe   // préfixe / chiffres (espaces milliers) / suffixe — découpage robuste
        if (!m) return;
    ...
    function showSection(nav, cinematic) {
      ...
      if (nav === 'produits') { view.innerHTML = produitsHTML(); bindProduits(); return; }
    ...
    const _noTour = /[?&]tour=off/.test(location.search);   // ?tour=off : démos et tests automatisés
    if (!SM.meta.onboarded) renderOnboarding();
    else { bootApp(); if (!SM.meta.tourDone && !_noTour) setTimeout(() => startTour(0), 900); }   // visite guidée tant qu'elle n'a pas été faite
    ...
    // bouton « ? » : explique l'écran où l'on se trouve, et permet de revoir la visite
    const _helpBtn = document.querySelector('[data-help]');
    if (_helpBtn) _helpBtn.addEventListener('click', ouvrirAide);
