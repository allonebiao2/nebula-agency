...
CANDIDATES = [
    Candidate("video_mamba", "MambaFx : zone M5 + cassure de structure M1", "YouTube MambaFx",
              videos.mamba_cassure, {"touches": [2, 3], "rr": [3.0, 5.0], "seance": ["ouverture", "toutes"], "be": [0.0, 1.0]},
              UNITES),
    Candidate("video_hugo", "Hugo FX : CRT H1 + swing M15 + discount M1", "YouTube Hugo FX",
              videos.hugo_crt, {"fib": [0.5, 0.62], "stop": ["swing", "079"], "seance": ["londres_ny", "toutes"], "be": [0.0, 1.0]},
              UNITES),
]
    ...
    for base in INSTRUMENTS:
        for tf in UNITES:
            ...
            for cand in CANDIDATES:
                ...
                versions = {}
                for nom_v, reglages in (("auteur", AUTEUR[cand.nom]), ("auteur_be", {**AUTEUR[cand.nom], "be": 1.0})):
                    t_v = banc.simuler(serie, cand.fabrique(serie, **reglages))
                    t_c = banc.simuler(chere, cand.fabrique(chere, **reglages))
                    versions[nom_v] = ({**banc.mesurer(serie, t_v), "par_annee": par_annee(serie, t_v),
                                        "couts_x1_5": banc.mesurer(chere, t_c)}, t_v)
                auteur, t_auteur = versions["auteur"]
                auteur_be, t_be = versions["auteur_be"]
                del chere
                ...
                (DOSSIER / f"{cle}.json").write_text(json.dumps(
                    {"candidate": cand.nom, "libelle": cand.libelle, "base": base, "tf": tf, "periode": periode,
                     ...
                     "auteur_be": auteur_be, "trades_R_auteur_be": [round(float(x), 4) for x in t_be.R],
                ...
                for variante, h, combis in (("auteur", auteur, 1), ("auteur_be", auteur_be, 1),
                ...
                print(f"  {cand.nom:12s} {base} {tf:3s} {periode}  AUTEUR "
                      ...
                      + (f" | POINT MORT {auteur_be['trades']} tr {100 * auteur_be['taux_reussite']:.1f} % "
                         f"{auteur_be['esperance_R']:+.3f} R" if auteur_be.get("trades") else "")
