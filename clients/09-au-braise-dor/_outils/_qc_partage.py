...
def main():
    ...
    def plats_ts():
        ...
        for m in re.finditer(r'\{ n: "([^"]*)"', ts):
            ...
            out.append((m.group(1), val("p"), val("p2"), val("pMax")))
        return out

    carte_ts = plats_ts()
    noms_ts = [x[0] for x in carte_ts]
    prix_ts = [x[1] for x in carte_ts]
    hauts = [max(x[1], x[2], x[3]) for x in carte_ts]
    ...
    fourchettes = [(n, hm) for n, p, p2, hm in carte_ts if hm]
    ...
    deux = [(n, p2) for n, p, p2, hm in carte_ts if p2]
    ...
    attendu = {n: {v for v in (p, p2, hm) if v > 0} for n, p, p2, hm in carte_ts}
