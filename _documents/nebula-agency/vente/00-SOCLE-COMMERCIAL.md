# SOCLE COMMERCIAL NEBULA · la source de vérité de la vente

> **À quoi sert ce document.** C'est la référence unique de tout ce qui se dit en vente chez
> NEBULA : les offres, les prix, ce qui est inclus, les délais, les gains du partenaire, les
> règles du jeu, et surtout **ce qu'on ne promet jamais**. L'avis de recrutement et les guides
> de vente en découlent tous.
> Si un chiffre change, **il change ici d'abord**, puis on répercute.
>
> **Version 2.0 · 2026-07-30 · Toutes les décisions validées par Mongazi**
> Confidentiel · usage interne NEBULA et partenaires actifs

---

## 1. Ce que NEBULA vend

NEBULA Agency est un **studio de solutions verticales** : on ne vend pas « des sites », on
conçoit l'outil digital dont un métier a besoin. Trois offres phares, plus un complément.

| # | Service | Ce que le client achète vraiment | Mise en service | Délai |
|---|---|---|---|---|
| 1 | **Catalogue Digital + QR Code** | Ses produits présentés une bonne fois pour toutes, commandables sur WhatsApp | **50 000 F** | 5 à 7 jours |
| 2 | **Vitrine Digitale + QR Code** | Sa crédibilité, son existence sur Google, son image de maison sérieuse | **150 000 F** | 5 à 7 jours |
| 3 | **Outil Digital sur mesure** | Le contrôle de son activité : suivi, commandes, stock, équipe, factures | **55 000 à 500 000 F** | selon projet |
| + | **QR Code Google Review** | Les avis Google de ses clients, sans effort | **30 000 F** | quelques jours |

### 1.1 L'abonnement, unique et simple

**20 000 F tous les 6 mois, modifications comprises.**

C'est **une seule ligne**, elle couvre l'hébergement, la sécurité **et les modifications de
contenu** (textes, prix, photos, ajout ou retrait de produits). Elle accompagne le Catalogue
et la Vitrine.

> L'abonnement est **par semestre**, jamais par mois. Un partenaire qui dit « 20 000 par mois »
> fait perdre la vente sur-le-champ. On dit : « 20 000 francs tous les six mois, soit environ
> 3 300 francs par mois, et vos modifications sont comprises dedans. »

**Ce que l'abonnement couvre :** modifications de contenu.
**Ce qu'il ne couvre pas :** une refonte du design, l'ajout de nouvelles fonctionnalités,
ou la création de nouvelles pages. Ce sont des devis à part.

Le **QR Code Google Review** est sans abonnement.

### 1.2 Ce qui est inclus, exactement

**Catalogue Digital · 50 000 F**
- Jusqu'à **20 produits** avec photo, description et prix
- Bouton de commande qui envoie directement sur son WhatsApp
- QR Code à imprimer et à coller
- Affichage sur téléphone, chargement rapide
- **Au-delà de 20 produits : 15 000 F par lot de 10 produits supplémentaires**
  *(un client à 35 produits paie 50 000 + 30 000 = 80 000 F)*

**Vitrine Digitale · 150 000 F**
- **Une page complète**, avec toutes les sections : accueil, présentation, services ou
  produits, galerie photos et vidéos, avis clients, contact, carte, bouton WhatsApp
- QR Code
- Visibilité Google (optimisation, sans garantie de position)
- **Nom de domaine personnalisé offert la 1ère année**, puis 16 000 F par an
- **Page supplémentaire : 30 000 F par page** *(un hub de 4 pages = 150 000 + 90 000 = 240 000 F)*

**Outil Digital sur mesure · 55 000 à 500 000 F**
- Le prix sort du **configurateur du site** (www.nebula-agency.online), jamais de la tête
  du partenaire
- Plancher 55 000 F, plafond 500 000 F
- Repères d'options : tableau de bord ~100 000 F · cockpit tout-en-un ~180 000 F ·
  factures normalisées ~120 000 F · application mobile ~300 000 F
- Projets Europe et international : **tarif export**, traité à part

### 1.3 L'escalier

Les 3 offres ne sont pas 3 produits concurrents, c'est **un escalier**.

```
   Outil métier      55 000 à 500 000 F     le contrôle et l'argent
        ↑
    Vitrine            150 000 F            la crédibilité
        ↑
   Catalogue            50 000 F            le temps et les commandes
```

Un commerçant méfiant dit oui à 50 000 F. Il ne dit pas oui à 150 000 F le premier jour.
Le Catalogue est la **porte d'entrée** : une fois livré et satisfaisant, la Vitrine se vend
presque toute seule, puis l'Outil.

**Un partenaire qui maîtrise l'escalier gagne trois fois sur le même client.**
Un partenaire qui attaque au prix fort brûle son prospect et ne revient jamais.

---

## 2. Comment le client paie

**Toujours à NEBULA, jamais au partenaire. Le partenaire ne touche jamais d'argent,
même liquide, même « pour rendre service ».**

| Offre | Modalité |
|---|---|
| **Catalogue** | **Paiement intégral**, pas d'acompte |
| **Vitrine** | **70 % au démarrage, 30 % à la mise en ligne.** Paiement intégral encouragé |
| **Outil métier** | **70 % au démarrage, 30 % à la livraison.** Paiement intégral encouragé |
| **QR Google Review** | Paiement intégral |

**Moyens acceptés :** Mobile Money sur le numéro officiel NEBULA, ou **virement bancaire**
selon le client (utile pour les entreprises qui ont besoin d'une trace comptable).

**Trois règles absolues :**

1. **Le partenaire ne communique jamais lui-même de coordonnées bancaires.** Il annonce que
   NEBULA les envoie directement au client. Le circuit reste sous votre contrôle.
2. **Toute somme reçue déclenche un message de confirmation de NEBULA au client dans les 24h.**
   C'est ce qui empêche toute zone grise entre ce que le partenaire dit et ce qui est réellement
   entré.
3. **Un virement bancaire n'est considéré comme payé qu'une fois constaté sur le compte**,
   pas quand le client affirme l'avoir fait.

**Le solde de 30 % est dû à la mise en ligne**, avant remise du lien final au client.
Le partenaire encourage toujours le paiement intégral, sans jamais le forcer.

**En cas de solde impayé : le site n'est pas mis en ligne.** Le travail est montré au client
en aperçu privé, mais la mise en ligne et le lien final ne partent qu'après règlement complet.
C'est la seule protection réelle, et elle est légitime puisque le solde est dû à la mise en ligne.

**Le Catalogue ne s'échelonne jamais.** À 50 000 F avec une livraison en 5 à 7 jours,
un paiement en deux fois crée plus d'impayés et de relances que de ventes. Le vendeur répond
« on démarre quand vous êtes prêt » et il rappelle.

---

## 3. Ce que le partenaire fait, et ce qu'il ne fait pas

| Le partenaire fait | NEBULA fait |
|---|---|
| Trouver et approcher les prospects | Concevoir et réaliser |
| Diagnostiquer le besoin | Héberger et sécuriser |
| Présenter, démontrer, répondre aux objections | Livrer en 5 à 7 jours |
| Conclure et faire payer NEBULA | Encaisser et déclencher la commission |
| Récupérer le brief complet | Assurer les corrections après livraison |

**Après la livraison :** le partenaire est le **premier interlocuteur** du client, et NEBULA
reste joignable directement. Les deux restent en contact sur chaque cas.
**Le partenaire peut écouter, rassurer et transmettre. Il ne peut jamais promettre une
correction ni un délai.** C'est NEBULA qui annonce ce qui sera fait et quand.

---

## 4. Ce que le partenaire gagne

### 4.1 Commission directe, par palier mensuel

Le palier se calcule sur le **nombre de ventes payées dans le mois en cours**, et il **repart
à zéro chaque mois**. Il s'applique à **tous les services**, Outil métier et QR Google Review
compris.

| Palier | Condition, dans le mois | Commission |
|---|---|---|
| **BRONZE** | par défaut | **30 %** |
| **ARGENT** | ses ventes **+ celles de ses filleuls directs** atteignent **3** | **40 %** |

**Il n'y a rien au-dessus de 40 %, et les 40 % ne sont jamais acquis.** Le compteur
**repart à zéro le 1er de chaque mois** : il faut refaire 3 ventes tous les mois pour
retrouver le taux haut. Un bon mois ne paie pas le mois suivant.

> **⛔ Il n'existe plus aucune commission de réseau.** Un parrain ne touche **rien** sur les
> ventes de ses filleuls. Ce que les filleuls changent, c'est **son palier** : leurs ventes
> du mois s'ajoutent aux siennes pour atteindre le seuil de 3 qui débloque les 40 %.
>
> C'est la phrase à dire en recrutement : **« personne ne gagne d'argent sur le dos de
> personne ici. »** Ce n'est plus un argument marketing, c'est devenu vrai.

### 4.2 Commission sur le récurrent (le portefeuille)

**20 % de chaque abonnement encaissé, tous les 6 mois, À VIE.**
Soit **4 000 F par client et par semestre**, à taux fixe, quel que soit son palier du mois,
renouvellements compris, **même s'il quitte le programme**.

| Son portefeuille | Ce que ça lui rapporte, sans rien vendre |
|---|---|
| 10 clients abonnés | 40 000 F par semestre |
| 30 clients abonnés | 120 000 F par semestre |
| 60 clients abonnés | 240 000 F par semestre |

**Deux règles qui encadrent le récurrent :**

1. **Il ne compte pas dans le palier.** Seules les ventes nouvelles font passer de 30 à
   40 %. Sans cette règle, un ancien partenaire serait au taux haut chaque mois sans
   prospecter personne, et le système récompenserait l'ancienneté au lieu de l'effort.
2. **Il est acquis à vie.** Ce qu'il construit lui appartient, même après son départ. C'est
   un argument de recrutement majeur, et c'est aussi la contrepartie de son engagement de
   non-démarchage de 24 mois (contrat, article 11.2).

**Le renouvellement du nom de domaine (16 000 F/an) ne donne lieu à aucune commission :**
c'est un coût refacturé au prix coûtant, pas une vente.

### 4.2 bis L'argument qui fait payer un renouvellement

**Le partenaire rappelle son client avant l'échéance, et lui dit la vérité technique :**

> « Votre abonnement arrive à échéance le [date]. S'il n'est pas réglé, **le site est
> coupé** : l'hébergement s'arrête, la sécurité aussi. Votre QR code ne mènera plus à rien
> et vos clients tomberont sur une page d'erreur. »

**Trois façons de le dire sans menacer**, parce que ce n'est pas une punition, c'est un fait :

1. **On annonce, on ne réclame pas.** « Je vous préviens à l'avance pour que ça ne vous
   surprenne pas », jamais « il faut payer ».
2. **On rappelle ce qu'on continue de faire pour lui** : héberger, sécuriser, et modifier
   son contenu autant qu'il veut. Il paie un service qui tourne, pas une taxe.
3. **On propose la marche suivante au même moment.** Un client content à l'échéance est le
   plus facile à faire monter d'un barreau de l'escalier.

⚠️ **Le partenaire y a un intérêt direct, et il faut le lui répéter :** un abonnement non
payé, c'est **4 000 F qu'il perd**, tous les six mois, pour toujours.

### 4.2 ter Le calendrier de coupure (tranché le 2026-08-02)

| Moment | Ce qui se passe |
|---|---|
| J-15 et J-3 | Rappels au client · alerte au partenaire à J-3 |
| **J+0 à J+7** | **Délai de courtoisie.** Le site reste en ligne, on annonce la date de coupure |
| **J+8** | **Le site est coupé.** Hébergement et sécurité interrompus, données conservées |
| M+6 | Dernier avertissement, puis suppression des données |

**Pourquoi 8 jours et pas 45 :** une échéance qu'on n'applique pas est une échéance qui
n'existe pas. Un client coupé au 45e jour apprend que la date est décorative et attendra
davantage au semestre suivant. Sept jours suffisent pour un virement, un déplacement ou une
absence ; au-delà, c'est un crédit gratuit. Et **le levier est maximal juste après
l'échéance**, tant que le QR est encore affiché dans la boutique et que le site tourne.

**Frais de réactivation : 5 000 F** pour remettre en ligne un site coupé, en plus du semestre
dû. **Aucun frais si le client règle pendant les 7 jours de courtoisie** : le délai est une
faveur qu'on peut perdre, pas un droit. Ces frais **ne portent aucune commission partenaire**,
ils couvrent la remise en ligne.

### 4.3 Les relances de renouvellement

**Double dispositif :** un rappel automatique part au client à l'échéance, puis **le partenaire
relance personnellement** si le client ne réagit pas.

> ⚠️ **Le rappel automatique est une infrastructure obligatoire, pas un confort.**
> Comme le récurrent est acquis à vie, les clients d'un partenaire parti n'ont plus personne
> pour les relancer : c'est l'automatisation qui portera cette collecte.
> À construire avec n8n + WhatsApp (stack existant).
>
> Pour le partenaire actif, l'échéance est **son propre argent** et le meilleur prétexte de
> reprise de contact qui existe.

Pour le partenaire actif, l'échéance de renouvellement est **le meilleur prétexte de reprise
de contact qui existe**, et le moment idéal pour vendre la marche supérieure de l'escalier.

### 4.4 Ce que les filleuls changent (et ce qu'ils ne rapportent pas)

**Aucune commission. Jamais. À aucune profondeur.**

Faire entrer quelqu'un dans le programme ne verse rien à son parrain. En échange, **les
ventes de ses filleuls directs s'ajoutent aux siennes** dans le compteur du mois : dès que
le total atteint **3**, tout son mois passe à **40 %**, rétroactivement, sur **ses** ventes
à lui.

| Ce qu'il fait | Ce que font ses filleuls | Son taux du mois |
|---|---|---|
| 1 vente | 0 | 30 % |
| 1 vente | 2 ventes | **40 %** sur sa vente |
| 3 ventes | 0 | **40 %** |
| 1 vente | 5 ventes | **40 %** sur sa vente |


**Une seule profondeur, et elle ne sert qu'au comptage** : les recrues de ses recrues ne
comptent nulle part.

> **Pourquoi ce système est meilleur que l'ancien.** Payer un parrain sur le travail d'un
> autre, c'est ce qui fait ressembler un programme à une pyramide, et c'est ce qui fait
> fuir les gens sérieux. Ici, on ne touche que sur **ce qu'on vend soi-même**. Amener
> quelqu'un de bon reste payant, mais par un autre chemin : **ça vous fait travailler à un
> taux plus élevé.** Et un parrain qui ne vend rien lui-même ne gagne rien du tout.

### 4.5 En francs

| Vente | 30 % | 40 % |
|---|---|---|---|
| QR Google Review · 30 000 F | 9 000 F | 12 000 F |
| Catalogue · 50 000 F | 15 000 F | 20 000 F |
| Vitrine · 150 000 F | 45 000 F | 60 000 F |
| Outil · 300 000 F | 90 000 F | 120 000 F |

**Mois type :** 4 Catalogues + 2 Vitrines = 6 ventes → 500 000 F de chiffre → **palier
ARGENT à 40 %** (le seuil de 3 est dépassé) → **200 000 F de commission**, plus 4 000 F par
client déjà au portefeuille. Le mois suivant, le compteur du palier repart de zéro.

### 4.6 Comment l'argent arrive

1. Le client paie NEBULA. 2. NEBULA marque le paiement. 3. Les commissions se créent
**automatiquement** (le vendeur, et son parrain s'il en a un), chacun est alerté. 4. Le partenaire clique
**« Réclamer »**. 5. **NEBULA paie sous 24 à 72h** en Mobile Money.

**Sur un projet en 70/30, la commission se verse en deux fois**, au rythme des encaissements.
**La vente compte dans le palier du mois dès le premier versement.**

> **Une vente = un lead payé.** Un contact intéressé qui ne paie pas ne génère aucune commission.

**Et une commission n'est acquise que sur de l'argent gardé.** Si un montant encaissé est
remboursé au client, la commission correspondante est reprise : annulée si elle n'est pas
encore versée, déduite des versements suivants si elle l'est déjà (contrat, article 6.7).

**L'exception qui rend la règle juste : si le remboursement vient d'une faute de NEBULA**
travail non livré, non conforme, abandonné : **la commission reste acquise au partenaire.**
Il a fait son travail, il n'a pas à payer notre erreur.

> Le remboursement doit rester exceptionnel : la règle est qu'**on reprend le travail jusqu'à
> satisfaction**. Un partenaire à qui on reprend une commission tous les mois est un partenaire
> qui part. Si le cas se répète, le problème est chez nous, pas chez lui.

### 4.7 Transparence dans l'espace partenaire

**Le classement est entièrement visible : ventes et gains de chacun.**
Voir qu'un collègue a gagné 200 000 F le mois dernier prouve que c'est possible, et c'est
l'un des rares moteurs gratuits dont dispose l'agence. Les **clients** de chacun restent
privés. En contrepartie de cette transparence, surveillez les tensions : si elles
apparaissent, masquer les gains se fait en une ligne.

**Pas d'adresse email @nebula-agency.online** : chaque partenaire utilise son email personnel.
Sa légitimité vient de son lien `/p/CODE`, de sa carte de visite générée et de son espace.

### 4.8 Les rangs (prestige, ventes cumulées à vie)

Les titres sont ceux d'une vraie maison de vente : ils disent le niveau réel de la personne,
pas un décor. Le rang **ne change pas la commission**, il dit qui vous avez en face.

| Ventes cumulées | Rang |
|---|---|
| 0 | Partenaire Junior |
| 1 à 5 | Conseiller |
| 6 à 15 | Conseiller Confirmé |
| 16 à 35 | Conseiller Senior |
| 36 à 65 | Chef de Secteur |
| 66 à 110 | Chef Régional |
| 111 à 150 | Directeur Commercial |
| **151 et plus** | **Directeur Associé** |

Au sommet du réseau, **Mongazi BIAO, Président Fondateur**, rang fixe.

*(Les insignes et les couleurs n'ont pas changé, seuls les titres ont été renommés
le 2026-08-02 : les anciens noms cosmiques faisaient jeu vidéo, pas société.)*

---

## 5. Les règles du jeu

### 5.1 À qui appartient un prospect

**Au premier partenaire qui l'enregistre dans son espace, pendant 60 jours.**
Passé ce délai sans vente, le prospect redevient libre.

**Conséquence pour le partenaire :** enregistrer un prospect le jour même de la visite n'est
pas une formalité administrative, c'est ce qui protège sa commission.

### 5.2 En cas de client mécontent

**NEBULA reprend le travail jusqu'à satisfaction. Le remboursement reste exceptionnel.**
La commission du partenaire reste acquise dans tous les cas.

> Cette garantie ne tient que si les partenaires ne survendent pas. C'est pour cette raison
> exacte que les interdits du §6 ne sont pas décoratifs.

### 5.3 Partenaire inactif

**2 mois sans vente :** NEBULA appelle pour comprendre et aider.
**4 mois sans vente :** accès désactivés, prospects libérés.

### 5.4 Un client déjà livré qui achète une deuxième offre

**La commission revient au partenaire d'origine, tant qu'il est actif.**

C'est ce qui donne tout son sens à l'escalier : un partenaire vend le Catalogue à 50 000 F
en sachant que la Vitrine à 150 000 F lui reviendra. Sans cette règle, plus personne ne
vendrait l'offre d'entrée.

### 5.5 Recrutement de filleuls

Un partenaire peut **inviter** qui il veut dès le premier jour, mais il **ne touche ses
le bénéfice de leurs ventes dans son palier qu'à partir de sa propre première vente**.

> On ne récompense jamais quelqu'un qui recrute sans avoir jamais vendu : il explique mal
> le métier, ses recrues abandonnent, et le programme dérive vers le recrutement au détriment
> de la vente.

### 5.6 Territoire

**La vente est libre partout** : Cotonou, tout le Bénin, l'étranger, la diaspora.
Seul le **recrutement** de la vague 1 est limité à Cotonou.

### 5.7 Certification : ce qu'un partenaire a le droit de vendre

> **Modifié le 2026-08-01, les guides et l'autorisation de vendre sont deux choses.**
> **Les cinq guides sont remis dès l'entrée**, à tout le monde. Un partenaire qui
> comprend l'escalier en entier vend mieux le premier barreau : il sait où il emmène
> son client. Ce qui reste échelonné, c'est le **droit de conclure seul**.

| Service | Guide remis | Droit de conclure seul |
|---|---|---|
| **Catalogue** et **QR Google Review** | dès l'entrée | Dès le jour 1, guide lu et quiz validé |
| **Vitrine** | dès l'entrée | Après sa **1ère vente livrée** |
| **Outil métier** | dès l'entrée | Après **3 ventes**, et **en binôme avec Mongazi sur ses 3 premiers dossiers** |

**Avant d'y avoir droit, un partenaire n'est pas bloqué : il passe la main.** Il détecte
le besoin, il obtient le rendez-vous, et Mongazi conclut avec lui. **Le partenaire garde
100 % de sa commission au taux plein**, l'apport vaut la vente.

Le binôme ne coûte rien : **le partenaire garde 100 % de sa commission au taux plein.**

### 5.8 Cadre écrit

**CGU du programme acceptées en ligne** (déjà en place, horodatées) **+ charte d'engagement
signée à la main.** La charte est envoyée en PDF, le partenaire la signe, en renvoie la photo
sur WhatsApp, et **ses accès ne sont créés qu'après réception**.

---

## 6. Ce qu'un partenaire ne promet JAMAIS

La section la plus importante du document. Une promesse de trop détruit un client, et un
client détruit coûte plus cher que dix ventes.

1. **Jamais un délai plus court que 5 à 7 jours** (après brief complet et paiement).
2. **Jamais « vous serez premier sur Google ».** On optimise, on ne garantit aucune position.
3. **Jamais un prix hors grille, jamais une remise décidée seul.** Toute demande remonte à NEBULA.
4. **Jamais promettre la facture normalisée comme acquise.** Au Bénin cela signifie conforme
   DGI (e-MECeF) : c'est une capacité vendable, la conformité s'étudie **au cas par cas**.
5. **Jamais modifier ni promettre de modifier un numéro WhatsApp** sur un site livré.
6. **Jamais promettre une fonctionnalité absente des guides** sans accord écrit de NEBULA.
7. **Jamais promettre une correction ou un délai après livraison.** On écoute, on transmet.
8. **Jamais encaisser d'argent, sous aucune forme.** Le client paie NEBULA, toujours.
9. **Jamais communiquer soi-même des coordonnées bancaires.** NEBULA les envoie.
10. **Jamais dénigrer un concurrent.** On montre notre travail, ça suffit.
11. **Jamais réutiliser les documents et visuels NEBULA** hors promotion de NEBULA.
12. **Jamais annoncer un prix ou un délai sur un Outil métier.** Le configurateur et le cadrage
    s'en chargent.

**Un manquement grave met fin au partenariat.**

---

## 7. Le Diagnostic Digital

**Notre porte d'entrée sur l'Outil métier, et le meilleur prétexte de visite qui existe.**

- **Gratuit, avec sa valeur affichée : « normalement 25 000 F, je vous l'offre. »**
- Durée : 45 à 60 minutes, chez le client
- **Le partenaire mène la collecte** avec la fiche de diagnostic
- **Mongazi restitue** les conclusions et le prix, avec le partenaire

**Pourquoi ça marche :** « Je viens vous vendre un logiciel » se fait éconduire.
« Je viens faire le diagnostic digital de votre affaire, c'est offert et ça prend 45 minutes »
entre partout. Et c'est honnête : le client repart avec quelque chose même s'il n'achète pas.

Méthode complète : `08-DIAGNOSTIC-DIGITAL.md`.

---

## 8. Ce qui n'est PAS dans le programme partenaires

**Le partenaire vend trois choses : le Catalogue, la Vitrine, l'Outil sur mesure**, plus le
QR Code Google Review. C'est le tableau des tarifs, et rien d'autre.

**Boussole** (l'outil de gestion financière du commerçant) reste hors du programme. Les
partenaires ne le vendent pas. S'ils rencontrent un cas manifestement Boussole, ils le
signalent à NEBULA sans engagement de leur part.

---

## 9. Le marché

Source : `_memoire/analyse-marche.md`

- **~730 000 PME sans site web** sur 5 pays d'Afrique de l'Ouest francophone
- **Moins de 10 %** des PME ciblables ont un site
- **70 % des PME vendent sur WhatsApp** sans rien derrière

**Les 4 barrières à l'achat :**

| Barrière | Ce qu'il pense | La réponse |
|---|---|---|
| **Méconnaissance** | « Un site, ça sert à quoi ? J'ai Facebook » | On montre un site livré |
| **Peur du coût** | « Un site c'est 500 000 F minimum » | Le Catalogue est à 50 000 F |
| **Méfiance** | « On m'a déjà pris de l'argent » | 9 réalisations en ligne, livraison 5 à 7 jours |
| **Personne ne lui a expliqué** | Il n'a jamais eu la conversation | C'est le travail du partenaire |

**Secteurs prioritaires à Cotonou :** beauté et cosmétique · mode et bijoux · restauration ·
photographie et événementiel · coaching et formation.

**À éviter au démarrage :** industrie B2B lourde, politique et ONG, santé pharmaceutique
et finance.

---

## 10. Les preuves à montrer

| Client | Secteur | Sert à vendre |
|---|---|---|
| **Djambar Team** · djambarteam.com | Bijouterie, Cotonou | Vitrine haut de gamme, multi-pages |
| **Grain d'Esthétique** · graindesthetique.com | Institut de beauté, Cotonou | Vitrine avec prise de rendez-vous |
| **Speed × Weinkeller** · speed-weinkeller.pages.dev | Cave à vins | Catalogue riche, 60 fiches produits |
| **Au Braisé d'Or** · au-braise-dor.pages.dev | Restaurant, Cotonou | Catalogue-menu, 48 plats commandables |
| **Miss Cakes** · miss-cakes.pages.dev | Pâtisserie | Catalogue commandable, une page |
| **HH Design** · hh-design.pages.dev | Ébénisterie | Vitrine avec fiches produit et devis |

Preuves de capacité sur l'Outil métier : **Boussole**, **Digital HSE**, **Vendora / NEXO**.

---

## 11. La vague 1

| Point | Décision |
|---|---|
| **Statut** | Partenaire commissionné, sur le programme existant |
| **Recrutement** | **Cotonou uniquement, AUCUN QUOTA DE PLACES** *(décision du 2026-08-01, remplace les 8 places)*, sélection sur entretien, **l'entretien reste obligatoire, c'est le seul filtre** |
| **Le vrai plafond** | Ce n'est plus le nombre de partenaires, c'est la **capacité à livrer en 5 à 7 jours**. Le jour où ce délai glisse, on ralentit le recrutement |
| **Candidatures** | Ouvertes **21 jours** après publication de l'annonce |
| **Formation** | **Visio Google Meet**, guides remis, charte signée avant les accès |
| **Animation** | **Point collectif hebdomadaire de 30 minutes** |
| **Objectif** | **30 ventes sur 90 jours** |
| **Primes et remises** | Aucune. La commission et le paiement en 24 à 72h suffisent |

> **Le point hebdomadaire n'est pas optionnel.** Sans prime de première vente ni offre de
> lancement, c'est le seul mécanisme qui retient un débutant pendant les semaines où il n'a
> encore rien vendu. C'est là que la majorité des vendeurs abandonnent, partout.

---

## 12. Fichiers liés

| Document | Contenu |
|---|---|
| `01-AVIS-DE-RECRUTEMENT.md` | Annonce, versions courtes, grille de sélection, entretien |
| `02-MANUEL-DU-PARTENAIRE.md` | Le métier : prospection, méthode en 7 temps, règles |
| `03-GUIDE-CATALOGUE.md` | Vente du Catalogue Digital |
| `04-GUIDE-VITRINE.md` | Vente de la Vitrine Digitale |
| `05-GUIDE-OUTIL-METIER.md` | Vente de l'Outil sur mesure |
| `06-ARSENAL-SCRIPTS.md` | Tous les messages prêts à copier |
| `07-MISE-EN-LIGNE.md` | Publication des supports, ordre de lancement |
| `08-DIAGNOSTIC-DIGITAL.md` | La consultation professionnelle et la détection des automatisations |
| `simulateur-commissions.html` | Simulateur de gains |
| `fiche-diagnostic.html` | Fiche de diagnostic à remplir chez le client |

---

## 13. Reste à répercuter

- [ ] **Site www.nebula-agency.online** : l'abonnement y est encore affiché à **15 000 F / 6 mois**
      (section Tarifs et cartes d'offres). À passer à **20 000 F, modifications comprises**.
- [ ] **Cerveau de NOVA** (`nebula-affilies/server.py`, `agency_brain()`) : à aligner sur les
      nouveaux prix et l'abonnement unique.
- [ ] **Anciens guides du back-office partenaires** (`seed_docs`) : ils poussent la Vitrine en
      premier, ce qui contredit l'escalier. À retirer ou réécrire.

---

*Document vivant. Toute modification de prix, de commission ou de règle se fait ICI en premier,
puis se répercute dans les guides.*
